...
import {
  BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer,
  PieChart, Pie, Cell, LineChart, Line, CartesianGrid, Legend, LabelList,
} from "recharts";
import { Skeleton } from "../components/ui/skeleton";
import { Badge } from "../components/ui/badge";
import {
...
/* ========== UPDATED: SalesPipelineChart with Data Labels ========== */
const SalesPipelineChart = ({ data, loading, totalLeads }) => {
  if (loading) return <Skeleton className="h-80 w-full rounded-lg" />;
  const hasData = data.some((d) => d.Open > 0 || d.Won > 0);
  const totalOpen = data.reduce((s, d) => s + d.Open, 0);
  ...
  // Custom tooltip showing counts and percentage of total leads for that month
  const CustomTooltip = ({ active, payload, label }) => {
    if (!active || !payload?.length) return null;
    const monthTotal = (payload[0]?.payload?.Open || 0) + (payload[0]?.payload?.Won || 0);
    return (
      <div className="bg-white p-3 rounded-lg shadow-lg border text-sm">
        ...
        {payload.map((p) => (
          <div key={p.dataKey} className="flex justify-between gap-4">
            ...
            {monthTotal > 0 && (
              <span className="text-gray-500">({((p.value / monthTotal) * 100).toFixed(0)}%)</span>
            )}
          </div>
        ))}
      </div>
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data} margin={{ top: 20, right: 10, left: 0, bottom: 20 }}>
                  ...
                  <Legend />
                  <Bar dataKey="Open" fill="#3B82F6" barSize={20} radius={[4,4,0,0]}>
                    <LabelList dataKey="Open" position="top" fill="#1e40af" fontSize={11} fontWeight="bold" formatter={(v) => (v > 0 ? v : "")} />
                  </Bar>
                  <Bar dataKey="Won"  fill="#10B981" barSize={20} radius={[4,4,0,0]}>
                    <LabelList dataKey="Won" position="top" fill="#065f46" fontSize={11} fontWeight="bold" formatter={(v) => (v > 0 ? v : "")} />
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
...
/* ========== UPDATED: DealDistributionChart with visible Open label ========== */
const DealDistributionChart = ({ data, loading, totalDeals }) => {
  if (loading) return <Skeleton className="h-80 w-full rounded-lg" />;
  const pieData = [
    { name: "Open", value: data.open, color: "#3B82F6" },
  ...
  // Custom label to always show name + percent if value > 0
...
/* ═══════════════════════════════════════════════════════════════
   MAIN DASHBOARD (unchanged except stage helpers already fixed)
═══════════════════════════════════════════════════════════════ */
const AdminDashboard = () => {
  const [loading,      setLoading]      = useState(true);
  const [error,        setError]        = useState(null);
