...
const AdminDashboard = () => {
  ...
  const fetchDeals = async (params) => {
    try {
      const res = await axios.get("http://localhost:5000/api/deals/getAll", { params });
      const deals = res.data || [];
      const counts = { open: 0, won: 0, lost: 0 };
  ...
  const invoiceChartData = Object.entries(groupedInvoices).map(([status, total]) => ({ status, total }));
  ...
  const mergedDealsData = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"].map((m) => {
    const found = dealsData.find((d) => d.month === m);
  ...
  return (
    <div className="p-6 space-y-6 min-h-screen">
        ...
        {["today","7days","month"].map((preset) => (
          <div key={preset} className="relative">
            <button
              onClick={() => applyPreset(preset)}
              className={`px-3 py-1 rounded-md text-sm font-medium ${
                activePreset === preset ? "bg-indigo-600 text-white" : "bg-white border"
              }`}
            >
              {preset === "today" ? "Today" : preset === "7days" ? "Last 7 Days" : "Month"}
            </button>

            {/* Month Dropdown */}
            {preset === "month" && activePreset === "month" && showMonthDropdown && (
              <div className="absolute mt-1 w-40 bg-white border rounded shadow z-10">
                {months.map((m) => (
                  <div
                    key={m.value}
                    onClick={() => { applyMonthFilter(m.value); setShowMonthDropdown(false); }}
                    className={`px-3 py-1 cursor-pointer hover:bg-indigo-100 ${
                      selectedMonth === m.value ? "bg-indigo-200 font-semibold" : ""
                    }`}
                  >
                    {m.label}
                  </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {summary.map((card, idx) => (
          ...
          >
            ...
            <div className="absolute -bottom-10 -left-10 w-32 h-32 bg-white opacity-20 rounded-full"></div>
            <CardHeader className="relative z-10">
              <CardTitle className="text-lg font-semibold tracking-wide">{card.title}</CardTitle>
            </CardHeader>
            <CardContent className="relative z-10 mt-4">
              <p className="text-4xl font-extrabold">{card.value}</p>
            </CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="relative overflow-hidden rounded-2xl shadow-xl bg-white p-6">
          <CardHeader>
            <CardTitle className="text-gray-800 text-lg font-semibold">Pipeline Board</CardTitle>
          </CardHeader>
          <CardContent className="h-72">
            {pipeline.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={pipeline} margin={{ top: 20, right: 20, left: 0, bottom: 20 }}>
                  <XAxis dataKey="stage" stroke="#6B7280" tick={{ fontSize: 12, fill: "#374151" }} />
                  <YAxis allowDecimals={false} stroke="#6B7280" tick={{ fontSize: 12, fill: "#374151" }} />
                  <Tooltip
                    cursor={{ fill: "rgba(243,244,246,0.5)" }}
                    contentStyle={{ backgroundColor: "white", borderRadius: "12px", border: "1px solid #e5e7eb", boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }}
                    formatter={(value) => [`${value} Deals`, "Deals"]}
                  />
                  <Bar dataKey="leads" fill="#3B82F6" barSize={40} radius={[8, 8, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <p className="text-gray-500 text-center">No pipeline data</p>
        ...
        <div className="relative overflow-hidden rounded-2xl shadow-xl bg-white p-6">
          <CardHeader>
            <CardTitle className="text-gray-800 text-lg font-semibold">Recent Invoices</CardTitle>
          </CardHeader>
          <CardContent className="h-72">
            {invoiceChartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={invoiceChartData} margin={{ top: 20, right: 20, left: 0, bottom: 20 }}>
                  <XAxis dataKey="status" stroke="#6B7280" tick={{ fontSize: 12, fill: "#374151" }} />
                  <YAxis allowDecimals={false} stroke="#6B7280" tick={{ fontSize: 12, fill: "#374151" }} />
                  <Tooltip
                  ...
                  <Bar dataKey="total" barSize={40} radius={[8, 8, 0, 0]}>
                    {invoiceChartData.map((entry, index) => {
                      const colors = { paid: "#34D399", unpaid: "#3B82F6", Pending: "#FBBF24", Overdue: "#EF4444" };
                      return <Cell key={index} fill={colors[entry.status] || "#6366F1"} />;
                    })}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="relative overflow-hidden rounded-2xl shadow-xl bg-white p-6">
          <CardHeader className="flex justify-between items-center">
            <CardTitle className="text-gray-800 text-lg font-semibold">Deals Overview</CardTitle>
          </CardHeader>
          <CardContent className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={mergedDealsData} margin={{ top: 20, right: 20, left: 0, bottom: 20 }}>
                <XAxis dataKey="month" stroke="#6B7280" />
                <YAxis allowDecimals={false} stroke="#6B7280" />
                <Tooltip contentStyle={{ backgroundColor: "white", borderRadius: "12px", border: "1px solid #e5e7eb" }} />
                <Line type="monotone" dataKey="open" stroke="#3B82F6" strokeWidth={2} dot name="Open" />
                <Line type="monotone" dataKey="won" stroke="#10B981" strokeWidth={2} dot name="Won" />
                <Line type="monotone" dataKey="lost" stroke="#EF4444" strokeWidth={2} dot name="Lost" />
                <Line type="monotone" dataKey="total" stroke="#8B5CF6" strokeWidth={2} dot name="Total" />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </div>
        ...
        <div className="relative overflow-hidden rounded-2xl shadow-xl bg-white p-6 flex flex-col items-center">
          <CardHeader>
            <CardTitle className="text-gray-800 text-lg font-semibold">Total Deals</CardTitle>
          </CardHeader>
          <div className="h-72 w-full flex justify-center items-center">
            <ResponsiveContainer>
              <PieChart>
                <Pie data={pieData} dataKey="value" cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={5}>
