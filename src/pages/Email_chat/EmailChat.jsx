...
/**
 * EmailChat.jsx  —  FIXED v2
 *
 * THE BUG: The axios `api` instance was reading the JWT from
 * localStorage.getItem("crm_token") but your login likely saves it under
 * a different key (e.g. "token", "authToken", "jwt", etc.).
 *
 * THE FIX:
 *   - getAuthToken() tries all common key names automatically
 *   - You can also set VITE_TOKEN_KEY in your .env to override
 *   - Every api call sends Authorization: Bearer <token>
 *   - Server uses the JWT to identify the user — no email param ever sent
 */
...
// ─── TOKEN READER ─────────────────────────────────────────────────────────────
// Reads the CRM JWT from localStorage.
// Tries common key names in order.  Override with VITE_TOKEN_KEY in .env
// e.g.  VITE_TOKEN_KEY=authToken
const TOKEN_KEY_CANDIDATES = [
  import.meta.env.VITE_TOKEN_KEY,   // e.g. "token", "authToken", "jwt"
  "token",
  "authToken",
  "auth_token",
  "jwt",
...
// ─── AXIOS INSTANCE ───────────────────────────────────────────────────────────
// Automatically attaches the CRM JWT Bearer token to every request.
// The server uses this token to identify the user — the client never
// sends an email address to select which Gmail account to use.
function createApiInstance(baseURL) {
  ...
  // Log 401 errors in dev to help debug token key mismatch
  instance.interceptors.response.use(
    (res) => res,
    (err) => {
      if (err.response?.status === 401 && import.meta.env.DEV) {
        console.warn(
          "[Gmail] 401 Unauthorized. JWT not found or invalid.\n" +
          "Tried keys:", TOKEN_KEY_CANDIDATES, "\n" +
          "localStorage keys:", Object.keys(localStorage),
          "\nSet VITE_TOKEN_KEY=<your_key> in .env to fix."
        );
      }
...
const EmailChat = () => {
  ...
  // Stable api instance — attaches JWT on every request
  ...
  const checkAuthStatus = async () => {
    ...
    try {
      ...
      // If 401 it means the CRM session expired — redirect to login
      if (err.response?.status === 401) {
        setError("Your session has expired. Please log in to the CRM again.");
  ...
  const markThreadAs = useCallback(async (threadId, action, value = true) => {
    ...
    try {
      const res = await api.post(endpointMap[action], bodyMap[action]);
        ...
        <DialogContent className="bg-white rounded-2xl shadow-xl max-w-md">
          ...
          <div className="p-6">
            ...
            <div className="flex items-center justify-center gap-3">
              <button onClick={() => setShowDisconnModal(false)}
                className="px-5 py-2.5 rounded-xl border border-gray-300 hover:bg-gray-50 text-gray-600 text-sm font-medium">Cancel</button>
              <button onClick={disconnectGmailFn}
                className="px-5 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white flex items-center gap-2 text-sm font-medium">
                <FaSignOutAlt size={13} /> Disconnect
              </button>
            </div>
