...
// export default EmailChat;//all working correctly live that usermail onel..
...
// // ============= CONSTANTS =============
// const GMAIL_ATTACH_LIMIT = 25 * 1024 * 1024; // 25 MB hard limit
...
// const saveToStorage = (key, data) => { try { localStorage.setItem(key, JSON.stringify(data)); } catch (e) {} };
// const loadFromStorage = (key, def) => { try { const s = localStorage.getItem(key); return s ? JSON.parse(s) : def; } catch (e) { return def; } };
// const loadSet = (key) => {
//   try { const s = localStorage.getItem(key); if (s) { const p = JSON.parse(s); if (Array.isArray(p)) return new Set(p); } } catch (e) {}
//   return new Set();
// };
...
// // ============= SUGGESTION INPUT (reusable) =============
// /**
//  * An email input field with a dropdown suggestion list.
//  * Suggestions are shown when the field is focused and there are results.
//  */
...
//     // Replace the last comma-separated token with the selected suggestion
...
//                 className="w-full text-left px-4 py-2.5 hover:bg-blue-50 flex items-center gap-2.5 text-sm text-gray-700 transition-colors"
//               >
...
//   const [labels, setLabels] = useState([]);
//   const [composeMode, setComposeMode] = useState("new");
//   // ── Suggestions for all three fields ──
...
//   // ── PERSIST ──
//   useEffect(() => { permanentlyDeletedThreadsRef.current = permanentlyDeletedThreads; try { localStorage.setItem(STORAGE_KEYS.DELETED_THREADS, JSON.stringify(Array.from(permanentlyDeletedThreads))); } catch(e) {} }, [permanentlyDeletedThreads]);
//   useEffect(() => { trashMovedThreadsRef.current = trashMovedThreads; try { localStorage.setItem(STORAGE_KEYS.TRASH_MOVED_THREADS, JSON.stringify(Array.from(trashMovedThreads))); } catch(e) {} }, [trashMovedThreads]);
//   useEffect(() => { readThreadsRef.current = readThreads; }, [readThreads]);
//   useEffect(() => { saveToStorage(STORAGE_KEYS.THREADS_CACHE, threadsCache); }, [threadsCache]);
//   useEffect(() => { saveToStorage(STORAGE_KEYS.PAGE_TOKENS, nextPageTokenCache); }, [nextPageTokenCache]);
//   useEffect(() => { saveToStorage(STORAGE_KEYS.TOTAL_COUNTS, totalEmailsCache); }, [totalEmailsCache]);
...
//   // ── Suggestions: debounce per-field ──
...
//       if (lastToken.length < 2) { setSuggestions(prev => ({ ...prev, [field]: [] })); return; }
//       try {
//         const res = await axios.get(`${API_BASE_URL}/gmail/suggestions`, { params: { query: lastToken, email: userEmail } });
//         if (res.data.success) setSuggestions(prev => ({ ...prev, [field]: res.data.data || [] }));
//       } catch { setSuggestions(prev => ({ ...prev, [field]: [] })); }
//     }, 400);
//   };

//   const handleComposeFieldChange = (field, value) => {
...
//     setComposeData(prev => ({ ...prev, [field]: value.trimStart() }));
...
//     setIsRefreshingCounts(true);
//     try {
//       const res = await axios.get(`${API_BASE_URL}/gmail/all-counts`, { params: { email: userEmail }, timeout: 10000 });
//       if (res.data.success) {
//         setActualCounts(res.data.counts);
//         setTotalEmailsCache(prev => ({ ...prev, ...res.data.counts }));
//         setLastFetchTime(prev => ({ ...prev, counts: Date.now() }));
...
//     setLoading(true); setLoadingLabel(label); setError("");
//     try {
//       const params = { maxResults: 20, label, email: userEmail };
//       if (loadMore && nextPageTokenCache[label]) params.pageToken = nextPageTokenCache[label];
//       const res = await axios.get(`${API_BASE_URL}/gmail/threads`, { params, signal: abortControllerRef.current.signal });
//       if (res.data.success) {
//         const currentReadSet = readThreadsRef.current;
...
//         }));
//         setNextPageTokenCache(prev => ({ ...prev, [label]: res.data.nextPageToken }));
//         if (label === "TRASH" && res.data.totalEstimate !== undefined) setActualCounts(prev => ({ ...prev, TRASH: res.data.totalEstimate }));
//         if (res.data.totalEstimate) setTotalEmailsCache(prev => ({ ...prev, [label]: res.data.totalEstimate }));
//         setLastFetchTime(prev => ({ ...prev, [label]: Date.now() }));
//         setSelectedThreads(new Set()); setShowBulkActions(false); setIsSelectAll(false);
//         setInitialLoadDone(true); setForceUpdate(p => p + 1);
...
//   const fetchLabels = async () => {
//     try {
//       const res = await axios.get(`${API_BASE_URL}/gmail/labels`, { params: { email: userEmail } });
//       if (res.data.success) setLabels(res.data.data);
//     } catch(e) {}
//   };
...
//   const fetchDrafts = async () => {
//     try {
//       const res = await axios.get(`${API_BASE_URL}/gmail/drafts`, { params: { maxResults: 20, email: userEmail } });
//       if (res.data.success) {
//         const drafts = (res.data.data || []).filter(d => !permanentlyDeletedThreadsRef.current.has(d.id)).map(d => ({ ...d, isDraft: true }));
//         setThreadsCache(prev => ({ ...prev, DRAFTS: drafts }));
//         setActualCounts(prev => ({ ...prev, DRAFTS: res.data.totalCount || drafts.length || 0 }));
//         setLastFetchTime(prev => ({ ...prev, DRAFTS: Date.now() }));
//       }
...
//       let res;
//       if (activeLabel === "DRAFTS") {
//         res = await axios.get(`${API_BASE_URL}/gmail/draft/${threadId}`, { params: { email: userEmail } });
//       } else {
//         res = await axios.get(`${API_BASE_URL}/gmail/thread/${threadId}`, { params: { email: userEmail } });
//       }
//       if (res.data.success) {
//         const msgs = Array.isArray(res.data.data) ? res.data.data : (res.data.data.messages || []);
//         setMessages(msgs);
//         setSelectedThread(threadId);
//         if (activeLabel !== "DRAFTS") await markThreadAs(threadId, "read", true);
//         fetchAllCountsFast();
//       } else { setError(res.data.error || "Failed to fetch thread/draft"); }
//     } catch(err) { setError(err.response?.data?.error || "Failed to fetch thread/draft"); }
//     finally { setLoading(false); }
//   };

//   const connectGmail = () => { if (authUrl) { setIsConnecting(true); window.location.href = authUrl; } };

//   const disconnectGmail = async () => {
//     setShowDisconnectModal(false);
//     try {
//       await axios.delete(`${API_BASE_URL}/gmail/disconnect`, { data: { email: userEmail } });
//       setReadThreads(new Set()); readThreadsRef.current = new Set();
//       setAuthStatus({ authenticated: false, message: "Gmail disconnected" });
//       setThreadsCache({ INBOX:[], UNREAD:[], STARRED:[], IMPORTANT:[], SENT:[], SPAM:[], TRASH:[], DRAFTS:[] });
//       setNextPageTokenCache({}); setTotalEmailsCache({}); setMessages([]); setSelectedThread(null); setUserEmail("");
//       setActualCounts({ INBOX:0, UNREAD:0, STARRED:0, IMPORTANT:0, SENT:0, SPAM:0, TRASH:0, DRAFTS:0 });
...
//   const handleFileSelect = (e) => {
//     const files = Array.from(e.target.files).filter(f => {
//       if (f.size > GMAIL_ATTACH_LIMIT) {
//         toast.error(`❌ "${f.name}" is ${formatFileSize(f.size)} — exceeds the 25 MB Gmail limit.`);
//         return false;
//       }
//       return true;
//     });
...
//   const totalAttachmentSize = selectedFiles.reduce((sum, f) => sum + f.size, 0);
...
//     const k = 1024, s = ["Bytes","KB","MB","GB"], i = Math.min(Math.floor(Math.log(bytes) / Math.log(k)), s.length - 1);
//     return `${parseFloat((bytes / Math.pow(k, i)).toFixed(2))} ${s[i]}`;
//   };
...
//   const sendEmail = async () => {
//     if (!composeData.to.trim()) { toast.error("Please enter recipient email address"); return; }
//     // Guard: check total size before sending
//     if (totalAttachmentSize > GMAIL_ATTACH_LIMIT) {
//       toast.error(`Total attachments (${formatFileSize(totalAttachmentSize)}) exceed the 25 MB Gmail limit.`);
//       return;
//     }
//     setSending(true); setSendingProgress(0); setError("");
//     try {
...
//         setSelectedFiles([]); setSuggestions({ to:[], cc:[], bcc:[] });
//         setShowCompose(false); fetchAllCountsFast();
//         if (["INBOX","SENT"].includes(activeLabel)) setTimeout(() => fetchThreads(activeLabel, false, true), 500);
//         else setTimeout(() => fetchThreads("INBOX", false, true), 1000);
//         setTimeout(() => setSendingProgress(0), 1000);
//       } else { throw new Error(res.data.error || "Failed to send email"); }
...
//       const res = await axios.post(`${API_BASE_URL}/gmail/${endpoint}`, body);
//       if (res.data.success) {
//         if (action !== "read") toast.success(res.data.message);
//         if (action === "star" && value && activeLabel === "INBOX") {
//           setThreadsCache(prev => {
//             const starred = prev.STARRED || [], toAdd = prev[activeLabel]?.find(t => t.id === threadId);
//             if (toAdd && !starred.some(t => t.id === threadId)) return { ...prev, STARRED: [{ ...toAdd, starred: true }, ...starred] };
...
//         case "star": body.star = value; await axios.post(`${API_BASE_URL}/gmail/bulk-star`, body); toast.success(`⭐ ${value ? "Starred" : "Unstarred"} ${ids.length} emails`); break;
//         case "delete": body.permanent = activeLabel === "TRASH"; const dr = await axios.post(`${API_BASE_URL}/gmail/bulk-delete`, body); toast.success(`🗑️ ${dr.data.message}`); break;
//         case "read": await Promise.all(ids.map(id => axios.post(`${API_BASE_URL}/gmail/thread/${id}/read`, { read: value, email: userEmail }))); break;
//         case "trash": await axios.post(`${API_BASE_URL}/gmail/bulk-trash`, body); toast.success(`Moved ${ids.length} emails to trash`); break;
//       }
//       setSelectedThreads(new Set()); setShowBulkActions(false); setIsSelectAll(false); fetchAllCountsFast();
//     } catch(err) { toast.error(`Failed to ${action} emails`); fetchAllCountsFast(); }
...
//     try {
//       if (activeLabel === "DRAFTS") {
//         await axios.delete(`${API_BASE_URL}/gmail/draft/${threadId}`, { data: { email: userEmail } });
//         toast.success("📝 Draft permanently deleted");
//         setThreadsCache(prev => { const u = { ...prev }; u.DRAFTS = (u.DRAFTS || []).filter(t => t.id !== threadId); return u; });
//         setActualCounts(prev => ({ ...prev, DRAFTS: Math.max(0, prev.DRAFTS - 1) }));
//         if (selectedThread === threadId) { setMessages([]); setSelectedThread(null); }
//         return;
//       }
...
//         permanentlyDeletedThreadsRef.current = new Set([...permanentlyDeletedThreadsRef.current, threadId]);
//         setPermanentlyDeletedThreads(prev => { const s = new Set(prev); s.add(threadId); return s; });
//         await axios.delete(`${API_BASE_URL}/gmail/thread/${threadId}`, { data: { email: userEmail } });
//         toast.success("🗑️ Thread permanently deleted");
//         trashMovedThreadsRef.current.delete(threadId);
//         setTrashMovedThreads(prev => { const s = new Set(prev); s.delete(threadId); return s; });
//       } else {
//         trashMovedThreadsRef.current = new Set([...trashMovedThreadsRef.current, threadId]);
//         setTrashMovedThreads(prev => { const s = new Set(prev); s.add(threadId); return s; });
//         await axios.post(`${API_BASE_URL}/gmail/thread/${threadId}/trash`, { email: userEmail });
//         toast.success("🗑️ Thread moved to trash");
//         setLastFetchTime(prev => ({ ...prev, TRASH: 0 }));
//       }
//       setThreadsCache(prev => { const u = { ...prev }; Object.keys(u).forEach(l => { u[l] = u[l].filter(t => t.id !== threadId); }); return u; });
...
//     } catch(err) {
//       if (activeLabel !== "DRAFTS") {
//         if (permanent) { permanentlyDeletedThreadsRef.current = new Set(Array.from(permanentlyDeletedThreadsRef.current).filter(id => id !== threadId)); setPermanentlyDeletedThreads(prev => { const s = new Set(prev); s.delete(threadId); return s; }); }
//         else { trashMovedThreadsRef.current = new Set(Array.from(trashMovedThreadsRef.current).filter(id => id !== threadId)); setTrashMovedThreads(prev => { const s = new Set(prev); s.delete(threadId); return s; }); }
//       }
//       toast.error(err.response?.data?.error || "Failed to delete");
//     } finally { setShowDeleteModal(false); setThreadToDelete(null); }
//   };
...
//   const downloadAttachment = async (messageId, attachment) => {
//     try {
//       const res = await axios.get(`${API_BASE_URL}/gmail/attachment/${messageId}/${attachment.id}`, { params: { email: userEmail }, responseType: "blob" });
//       const url = window.URL.createObjectURL(new Blob([res.data])), link = document.createElement("a");
//       link.href = url; link.setAttribute("download", attachment.filename);
//       document.body.appendChild(link); link.click(); link.remove(); window.URL.revokeObjectURL(url);
//       toast.success(`📥 Downloaded ${attachment.filename}`);
...
//   const clearCompose = () => {
//     setComposeData({ to:"", cc:"", bcc:"", subject:"", message:"" });
//     setSelectedFiles([]); setComposeMode("new"); setSuggestions({ to:[], cc:[], bcc:[] });
//   };
//   const handleBackToList = () => { setSelectedThread(null); setMessages([]); };
//   const handleShowUnreadClick = () => handleLabelClick({ id: "UNREAD" });
...
//   const currentThreads = (threadsCache[activeLabel] || []).filter(t => {
//     try { return !permanentlyDeletedThreadsRef.current.has(t.id) && (activeLabel === "TRASH" ? true : !trashMovedThreadsRef.current.has(t.id)); } catch(e) { return true; }
//   });
...
//             {/* ── Cc field with suggestions ── */}
//             <EmailInputWithSuggestions
//               label="Cc" field="cc"
//               value={composeData.cc}
//               onChange={handleComposeFieldChange}
//               suggestions={suggestions.cc}
//               onSelectSuggestion={handleSelectSuggestion}
//               placeholder="Cc (optional, comma separated)"
//             />

//             {/* ── Bcc field with suggestions ── */}
//             <EmailInputWithSuggestions
//               label="Bcc" field="bcc"
//               value={composeData.bcc}
//               onChange={handleComposeFieldChange}
//               suggestions={suggestions.bcc}
//               onSelectSuggestion={handleSelectSuggestion}
...
//                     (max 25 MB per file
//                     {selectedFiles.length > 0 && ` · total ${formatFileSize(totalAttachmentSize)}`})
//                   </span>
//                   {totalAttachmentSize > GMAIL_ATTACH_LIMIT && (
//                     <span className="text-red-500 text-xs font-semibold">⚠ Over Gmail limit!</span>
//                   )}
//                 </label>
//                 <button onClick={triggerFileInput}
//                   className="text-xs text-white px-3 py-1.5 rounded-lg flex items-center gap-1.5" style={{ background: "#1a73e8" }}>
...
//                 <div className="space-y-2">
//                   {selectedFiles.map((f, i) => {
//                     const overLimit = f.size > GMAIL_ATTACH_LIMIT;
//                     return (
//                       <div key={i} className={`flex items-center justify-between p-2.5 bg-white rounded-lg border ${overLimit ? "border-red-300 bg-red-50" : "border-gray-100"}`}>
//                         <div className="flex items-center gap-2 flex-1 min-w-0">
//                           <span>{getFileIcon(f)}</span>
//                           <div className="min-w-0">
//                             <p className={`text-xs font-medium truncate ${overLimit ? "text-red-700" : "text-gray-700"}`}>{f.name}</p>
//                             <p className={`text-xs ${overLimit ? "text-red-500 font-semibold" : "text-gray-400"}`}>
//                               {formatFileSize(f.size)}{overLimit && " — exceeds 25 MB limit"}
//                             </p>
//                           </div>
//                         </div>
//                         <button onClick={() => removeFile(i)} className="ml-2 text-red-400 hover:text-red-600 p-1">
...
//                   <FaPaperclip size={28} className="text-gray-300 mx-auto mb-2" />
//                   <p className="text-xs text-gray-400">Drop files here or click Add</p>
//                   <p className="text-xs text-gray-300 mt-1">Maximum 25 MB per file</p>
//                 </div>
//               )}
//             </div>
...
//             <div className="flex gap-2">
//               <button onClick={() => setShowCompose(false)} className="px-4 py-2 text-gray-500 border border-gray-200 rounded-lg hover:bg-gray-50 text-sm">Cancel</button>
//               <button onClick={sendEmail} disabled={sending || !composeData.to.trim() || totalAttachmentSize > GMAIL_ATTACH_LIMIT}
//                 className="px-4 py-2 text-white rounded-lg disabled:opacity-50 flex items-center gap-1.5 text-sm"
//                 style={{ background: (sending || totalAttachmentSize > GMAIL_ATTACH_LIMIT) ? "#ccc" : "linear-gradient(135deg,#1a73e8,#1557b0)" }}>
//                 {sending ? <><FaSpinner className="animate-spin" size={13} />Sending...</> : <><FaPaperPlane size={13} />Send</>}
//               </button>
//             </div>
//           </div>
...
// export default EmailChat;//ok for all
...
const EmailChat = () => {
  ...
  // ── KEY FIX: clearCompose resets all compose state ──
  const clearCompose = () => {
    setComposeData({ to:"", cc:"", bcc:"", subject:"", message:"" });
    setSelectedFiles([]);
    setComposeMode("new");
  ...
  // ── KEY FIX: handler called when compose dialog closes for ANY reason ──
  const handleComposeOpenChange = (open) => {
    if (!open) {
      setShowCompose(false);
      clearCompose(); // Always clear data when modal closes
    } else {
      setShowCompose(true);
    }
  };
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden">
      ...
      {/* ── COMPOSE MODAL ──
          KEY FIX: onOpenChange now calls handleComposeOpenChange which always
          runs clearCompose() when the modal closes — whether via Cancel button,
          clicking the X icon, or clicking outside the modal. */}
      <Dialog open={showCompose} onOpenChange={handleComposeOpenChange}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-white rounded-2xl shadow-xl">
          <DialogHeader className="border-b border-gray-100 pb-4">
            <DialogTitle className="flex items-center gap-2 text-gray-800 text-lg font-semibold">
              {composeMode === "reply" ? <FaReply size={15} /> : composeMode === "forward" ? <FaForward size={15} /> : <FaEdit size={15} />}
          ...
          <div className="flex justify-between items-center mt-5 pt-4 border-t border-gray-100">
            ...
            <div className="flex gap-2">
              {/* KEY FIX: Cancel button now calls handleComposeOpenChange(false) to clear data */}
              <button
