...
// ─── STORAGE HELPERS ─────────────────────────────────
// ✅ FIX: Storage keys are scoped to BOTH crmUserId AND gmailEmail
// This prevents data bleed between different CRM users on the same browser
const makeStorageKey = (crmUserId, email, suffix) =>
  crmUserId && email ? `gmail_${suffix}_${crmUserId}_${email}` : null;

const save    = (key, data) => { try { if (key) localStorage.setItem(key, JSON.stringify(data)); } catch {} };
const load    = (key, def)  => { try { if (!key) return def; const s = localStorage.getItem(key); return s ? JSON.parse(s) : def; } catch { return def; } };
const loadSet = (key)       => { try { if (!key) return new Set(); const s = localStorage.getItem(key); return s ? new Set(JSON.parse(s)) : new Set(); } catch { return new Set(); } };

const clearUserStorage = (crmUserId, email) => {
  if (!crmUserId || !email) return;
  ["threads","pageTokens","totalCounts","actualCounts","activeLabel","lastFetch","selectedThread","messages","deleted","trashMoved"]
    .forEach(p => { try { localStorage.removeItem(makeStorageKey(crmUserId, email, p)); } catch {} });
};

// ✅ FIX: Auth state is per-CRM-user — keyed by CRM user ID
const makeAuthKey  = (crmUserId) => crmUserId ? `gmail_auth_v4_${crmUserId}` : null;
const makeEmailKey = (crmUserId) => crmUserId ? `gmail_email_v4_${crmUserId}` : null;
const SIDEBAR_KEY  = "gmail_sidebar_v3";
...
const EmailChat = () => {
  ...
  // ✅ FIX: Get CRM user from localStorage (set during CRM login)
  const crmUser = JSON.parse(localStorage.getItem("user") || "{}");
  const crmUserId = crmUser?._id || null;

  // ✅ FIX: Create axios instance with auth header pre-applied
  // All Gmail API calls automatically carry the CRM JWT token
  const API_BASE = import.meta.env.VITE_API_URL || "http://localhost:5000/api";
  const SI_URI   = import.meta.env.VITE_SI_URI  || "http://localhost:5000";
  ...
  // ── AUTH ──
  const authKey   = makeAuthKey(crmUserId);
  const emailKey  = makeEmailKey(crmUserId);

  const [authStatus,    setAuthStatus]    = useState(() => load(authKey,  { authenticated: false, message: "" }));
  const [userEmail,     setUserEmail]     = useState(() => load(emailKey, ""));
  const [authUrl,       setAuthUrl]       = useState("");
  const [isConnecting,  setIsConnecting]  = useState(false);
  const [error,         setError]         = useState("");
  const [authLoading,   setAuthLoading]   = useState(false);
  ...
  useEffect(() => {
    ...
    if (crmUserId && userEmail) save(makeStorageKey(crmUserId, userEmail, "deleted"), [...deletedSet]);
  }, [deletedSet, crmUserId, userEmail]);

  useEffect(() => {
    trashRef.current = trashSet;
    if (crmUserId && userEmail) save(makeStorageKey(crmUserId, userEmail, "trashMoved"), [...trashSet]);
  }, [trashSet, crmUserId, userEmail]);

  useEffect(() => { readRef.current = readSet; }, [readSet]);

  // ─── PERSIST ──────────────────────────────────────
  useEffect(() => { if (crmUserId && userEmail) save(makeStorageKey(crmUserId, userEmail, "threads"),      threadsCache);  }, [threadsCache,  crmUserId, userEmail]);
  useEffect(() => { if (crmUserId && userEmail) save(makeStorageKey(crmUserId, userEmail, "pageTokens"),   nextPageTokens);}, [nextPageTokens, crmUserId, userEmail]);
  useEffect(() => { if (crmUserId && userEmail) save(makeStorageKey(crmUserId, userEmail, "actualCounts"), actualCounts);  }, [actualCounts,  crmUserId, userEmail]);
  useEffect(() => { if (crmUserId && userEmail) save(makeStorageKey(crmUserId, userEmail, "activeLabel"),  activeLabel);   }, [activeLabel,   crmUserId, userEmail]);
  useEffect(() => { if (crmUserId && userEmail) save(makeStorageKey(crmUserId, userEmail, "lastFetch"),    lastFetchTime); }, [lastFetchTime, crmUserId, userEmail]);
  useEffect(() => { if (authKey)  save(authKey,  authStatus);       }, [authStatus,       authKey]);
  useEffect(() => { if (emailKey) save(emailKey, userEmail);        }, [userEmail,        emailKey]);
  useEffect(() => { save(SIDEBAR_KEY, sidebarCollapsed); }, [sidebarCollapsed]);

  // ─── LOAD CACHE ON EMAIL CHANGE ───────────────────
  useEffect(() => {
    if (!userEmail || !crmUserId) return;
    setThreadsCache(load(makeStorageKey(crmUserId, userEmail, "threads"),      EMPTY_CACHE()));
    setNextPageTokens(load(makeStorageKey(crmUserId, userEmail, "pageTokens"), {}));
    setActualCounts(load(makeStorageKey(crmUserId, userEmail, "actualCounts"), EMPTY_COUNTS()));
    setActiveLabel(load(makeStorageKey(crmUserId, userEmail, "activeLabel"),   "INBOX"));
    setLastFetchTime(load(makeStorageKey(crmUserId, userEmail, "lastFetch"),   {}));
    setSelectedThread(null); setMessages([]);
    const del = loadSet(makeStorageKey(crmUserId, userEmail, "deleted"));
    const tr  = loadSet(makeStorageKey(crmUserId, userEmail, "trashMoved"));
    setDeletedSet(del); deletedRef.current = del;
    setTrashSet(tr);    trashRef.current   = tr;
    setReadSet(new Set()); readRef.current  = new Set();
  }, [userEmail, crmUserId]);
  ...
  const doFetchCounts = useCallback(async (emailArg) => {
    ...
    if (!email) return;
    try {
      const res = await authAxios().get(`/gmail/all-counts`, { params: { email }, timeout: 12000 });
      if (res.data.success) {
        setActualCounts(res.data.counts);
        setLastFetchTime(p => ({ ...p, counts: Date.now() }));
      }
    } catch (err) {
      // ✅ FIX: Handle 401 gracefully — token expired or missing
      if (err.response?.status === 401) {
        setAuthStatus({ authenticated: false, message: "Session expired. Please log in again." });
      }
    }
  }, [authAxios, userEmail]);
  ...
  const fetchLabel = useCallback(async ({
    ...
    if (!force && !loadMore && label !== "UNREAD") {
      const cachedData = load(makeStorageKey(crmUserId, email, "threads"), EMPTY_CACHE());
      const cached     = cachedData[label] || [];
      const times      = load(makeStorageKey(crmUserId, email, "lastFetch"), {});
      const age        = times[label] ? Date.now() - times[label] : Infinity;
      if (cached.length > 0 && age < 120000) {
        const filtered = cached
          .filter(t => !del.has(t.id))
    ...
    try {
      const params = { maxResults: 20, label, email };
      if (loadMore) {
        const tokens = load(makeStorageKey(crmUserId, email, "pageTokens"), {});
        if (tokens[label]) params.pageToken = tokens[label];
      }
      const res = await authAxios().get(`/gmail/threads`, { params, signal, timeout: 15000 });
  ...
  const fetchDrafts = useCallback(async (emailArg) => {
    ...
    setListLoading(true); setListLoadLabel("DRAFTS");
    try {
      const res = await authAxios().get(`/gmail/drafts`, { params: { maxResults: 20, email } });
      if (res.data.success) {
        const drafts = (res.data.data || [])
          .filter(d => !deletedRef.current.has(d.id))
          .map(d => ({ ...d, isDraft: true }));
  ...
  const bootstrapForEmail = useCallback(async (email) => {
    ...
    try {
      const res = await authAxios().get(`/gmail/labels`, { params: { email } });
      if (res.data.success) setLabels(res.data.data || []);
    } catch {}
  }, [doFetchCounts, fetchLabel, authAxios]);
  ...
  useEffect(() => {
    ...
    // ✅ FIX: If no CRM user in localStorage, force redirect to login
    if (!crmUserId) {
      navigate("/login", { replace: true });
      return;
    }

    const cachedAuth  = load(authKey,  { authenticated: false });
    const cachedEmail = load(emailKey, "");
  ...
  const checkAuthStatus = async () => {
    ...
    try {
      const cachedEmail = load(emailKey, "");
      const res = await authAxios().get(`/gmail/auth-status`, {
        params: cachedEmail ? { email: cachedEmail } : {},
      });
      setAuthStatus(res.data);
      if (res.data.authenticated && res.data.email) {
  ...
  const fetchAuthUrl = async () => {
    try {
      // ✅ FIX: Auth URL request must carry JWT so backend embeds crmUserId in state
      const res = await authAxios().get(`/gmail/auth-url`);
      if (res.data.success) setAuthUrl(res.data.url);
      else setError(res.data.error || "Failed to get authentication URL");
    } catch (err) {
      if (err.response?.status === 401) {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (activeLabel !== "DRAFTS") {
          ...
          setActualCounts(p => ({ ...p, UNREAD: Math.max(0, p.UNREAD - 1) }));
          authAxios().post(`/gmail/thread/${threadId}/read`, { read: true, email: userEmail }).catch(() => {});
        }
        doFetchCounts(userEmail);
      }
    } catch (err) {
  ...
  }, [activeLabel, authAxios, userEmail, threadLoading, doFetchCounts]);
  ...
  const handleLabelClick = useCallback(async (labelId) => {
    ...
    if (labelId === "TRASH") {
      setTrashSet(new Set()); trashRef.current = new Set();
      if (crmUserId && userEmail) localStorage.removeItem(makeStorageKey(crmUserId, userEmail, "trashMoved"));
    }
    ...
    setShowMobileSidebar(false);
  }, [userEmail, crmUserId, threadsCache, lastFetchTime, fetchLabel, fetchDrafts]);
  ...
  const markThreadAs = useCallback(async (threadId, action, value = true) => {
    ...
    if (action === "star")
      setActualCounts(p => ({ ...p, STARRED: value ? p.STARRED + 1 : Math.max(0, p.STARRED - 1) }));
    const endpointMap = { star:`/gmail/thread/${threadId}/star`, important:`/gmail/thread/${threadId}/important`, spam:`/gmail/thread/${threadId}/spam`, trash:`/gmail/thread/${threadId}/trash` };
    const bodyMap     = { star:{ star:value }, important:{ important:value }, spam:{ spam:value }, trash:{} };
    try {
      const res = await authAxios().post(endpointMap[action], { email: userEmail, ...bodyMap[action] });
      if (res.data.success) { toast.success(res.data.message); doFetchCounts(userEmail); }
      else throw new Error(res.data.error);
    } catch {
      setThreadsCache(prevCache); setActualCounts(prevCounts);
  ...
  }, [threadsCache, actualCounts, authAxios, userEmail, doFetchCounts]);
  ...
  const handleBulkAction = useCallback(async (action, value = true) => {
    ...
    try {
      if (action === "star") {
        setThreadsCache(p => ({ ...p, [activeLabel]: (p[activeLabel]||[]).map(t => selectedThreads.has(t.id) ? { ...t, starred:value } : t) }));
        await authAxios().post(`/gmail/bulk-star`, { threadIds:ids, star:value, email:userEmail });
        toast.success(`⭐ ${value?"Starred":"Unstarred"} ${ids.length} emails`);
      }
      if (action === "trash") {
        setThreadsCache(p => ({ ...p, [activeLabel]: (p[activeLabel]||[]).filter(t => !selectedThreads.has(t.id)) }));
        await authAxios().post(`/gmail/bulk-trash`, { threadIds:ids, email:userEmail });
        toast.success(`Moved ${ids.length} emails to trash`);
      }
      if (action === "delete") {
        setThreadsCache(p => ({ ...p, [activeLabel]: (p[activeLabel]||[]).filter(t => !selectedThreads.has(t.id)) }));
        await authAxios().post(`/gmail/bulk-delete`, { threadIds:ids, permanent:activeLabel==="TRASH", email:userEmail });
        toast.success(`Deleted ${ids.length} emails`);
      }
      if (action === "read") {
        setReadSet(p => { const s = new Set(p); ids.forEach(id => value ? s.add(id) : s.delete(id)); return s; });
        setThreadsCache(p => ({ ...p, [activeLabel]: (p[activeLabel]||[]).map(t => selectedThreads.has(t.id) ? { ...t, unread:!value } : t) }));
        await Promise.all(ids.map(id => authAxios().post(`/gmail/thread/${id}/read`, { read:value, email:userEmail })));
      }
      setSelectedThreads(new Set()); setShowBulkActions(false);
      doFetchCounts(userEmail);
    } catch { toast.error(`Failed to ${action}`); }
  }, [selectedThreads, activeLabel, authAxios, userEmail, doFetchCounts]);

  // ─── DELETE THREAD ─────────────────────────────────
  const deleteThreadFn = useCallback(async (threadId, permanent = false) => {
    try {
      if (activeLabel === "DRAFTS") {
        await authAxios().delete(`/gmail/draft/${threadId}`, { data:{ email:userEmail } });
        toast.success("📝 Draft deleted");
        setThreadsCache(p => ({ ...p, DRAFTS:(p.DRAFTS||[]).filter(t => t.id !== threadId) }));
        setActualCounts(p => ({ ...p, DRAFTS:Math.max(0, p.DRAFTS-1) }));
        if (selectedThread === threadId) { setSelectedThread(null); setMessages([]); }
      ...
      if (permanent) {
        setDeletedSet(p => { const s = new Set(p); s.add(threadId); return s; });
        await authAxios().delete(`/gmail/thread/${threadId}`, { data:{ email:userEmail } });
        toast.success("🗑️ Permanently deleted");
      } else {
        setTrashSet(p => { const s = new Set(p); s.add(threadId); return s; });
        await authAxios().post(`/gmail/thread/${threadId}/trash`, { email:userEmail });
        toast.success("🗑️ Moved to trash");
      }
      setThreadsCache(p => { const u = {...p}; Object.keys(u).forEach(l => { u[l] = u[l].filter(t => t.id !== threadId); }); return u; });
      if (selectedThread === threadId) { setSelectedThread(null); setMessages([]); }
  ...
  }, [activeLabel, authAxios, userEmail, selectedThread, doFetchCounts]);

  // ─── DISCONNECT ────────────────────────────────────
  const disconnectGmail = async () => {
    setShowDisconnModal(false);
    try {
      await authAxios().delete(`/gmail/disconnect`, { data:{ email:userEmail } });
      clearUserStorage(crmUserId, userEmail);
      setUserEmail(""); setAuthStatus({ authenticated:false, message:"Gmail disconnected" });
      setThreadsCache(EMPTY_CACHE()); setActualCounts(EMPTY_COUNTS());
      setMessages([]); setSelectedThread(null);
      setDeletedSet(new Set()); deletedRef.current = new Set();
  ...
  const handleSuggestionFetch = (field, query) => {
    ...
    suggestTimers.current[field] = setTimeout(async () => {
      ...
      if (token.length < 2) { setSuggestions(p => ({ ...p, [field]:[] })); return; }
      try {
        const res = await authAxios().get(`/gmail/suggestions`, { params:{ query:token, email:userEmail } });
        if (res.data.success) setSuggestions(p => ({ ...p, [field]: res.data.data||[] }));
      } catch { setSuggestions(p => ({ ...p, [field]:[] })); }
    }, 400);
  };
  ...
  const sendEmail = async () => {
    ...
    try {
      ...
      fd.append("email",   userEmail);
      selectedFiles.forEach(f => fd.append("attachments", f));
      const res = await authAxios().post(`/gmail/send`, fd, {
        headers: { "Content-Type": "multipart/form-data" },
        onUploadProgress: e => { if (e.total) setSendProgress(Math.round((e.loaded*100)/e.total)); },
        timeout: 300000,
      });
  ...
  const saveAsDraft = async () => {
    ...
    try {
      ...
      fd.append("message", composeData.message||""); fd.append("email", userEmail);
      selectedFiles.forEach(f => fd.append("attachments", f));
      const res = await authAxios().post(`/gmail/draft`, fd, { headers:{ "Content-Type":"multipart/form-data" } });
      if (res.data.success) {
        toast.success("📝 Draft saved!"); clearCompose(); setShowCompose(false);
        doFetchCounts(userEmail);
        if (activeLabel === "DRAFTS") setTimeout(() => fetchDrafts(userEmail), 500);
  ...
  const downloadAttachment = async (messageId, attachment) => {
    try {
      const res = await authAxios().get(`/gmail/attachment/${messageId}/${attachment.id}`, {
        params:{ email:userEmail }, responseType:"blob",
      });
      const url  = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement("a");
