...
// ─── TOKEN HELPER ─────────────────────────────────────
// 🔒 Tries every common key name your CRM login might use.
//    Check your login API response and the key you use in localStorage —
//    add it here if it's not already listed.
...
// 🔒 Decode CRM user ID from the JWT payload (works with any standard JWT)
function getCrmUserIdFromToken() {
  try {
    const token = getAuthToken();
    if (!token) return null;
    const payload = JSON.parse(atob(token.split(".")[1]));
    // Support all common claim names for user ID
...
// ─── STORAGE HELPERS ─────────────────────────────────
// 🔒 Cache is keyed by CRM user ID so data never leaks between users
const makeCacheKey  = (uid, suffix) => uid ? `gmail_${suffix}_${uid}` : null;
const save    = (key, data) => { try { if (key) localStorage.setItem(key, JSON.stringify(data)); } catch { } };
const load    = (key, def)  => { try { if (!key) return def; const s = localStorage.getItem(key); return s ? JSON.parse(s) : def; } catch { return def; } };
const loadSet = (key)       => { try { if (!key) return new Set(); const s = localStorage.getItem(key); return s ? new Set(JSON.parse(s)) : new Set(); } catch { return new Set(); } };
const clearUserStorage = (uid) => {
  if (!uid) return;
  ["threads","pageTokens","totalCounts","actualCounts","activeLabel","lastFetch","deleted","trashMoved"]
    .forEach(p => { try { localStorage.removeItem(makeCacheKey(uid, p)); } catch { } });
};
...
const EmailChat = () => {
  ...
  // 🔒 CRM user ID decoded from JWT — used as localStorage cache key
  const crmUserId = getCrmUserIdFromToken();

  // ─── Secure axios instance — every request carries the JWT ────────────────
  // This is the ONLY place where auth headers are set. No email params needed.
  const secureApi = useCallback(() => {
    const token = getAuthToken();
    return axios.create({
      baseURL: API_BASE,
  ...
  // ── AUTH ──
  const [authStatus,     setAuthStatus]     = useState(() => load(AUTH_KEY, { authenticated: false, message: "" }));
  const [connectedEmail, setConnectedEmail] = useState(""); // display only
  const [authUrl,        setAuthUrl]        = useState("");
  const [isConnecting,   setIsConnecting]   = useState(false);
  const [error,          setError]          = useState("");
  const [authLoading,    setAuthLoading]    = useState(false);
  ...
  useEffect(() => {
    ...
    if (crmUserId) save(makeCacheKey(crmUserId,"deleted"), [...deletedSet]);
  }, [deletedSet, crmUserId]);
  useEffect(() => {
    trashRef.current = trashSet;
    if (crmUserId) save(makeCacheKey(crmUserId,"trashMoved"), [...trashSet]);
  }, [trashSet, crmUserId]);
  useEffect(() => { readRef.current = readSet; }, [readSet]);

  // ─── PERSIST ──────────────────────────────────────
  useEffect(() => { if (crmUserId) save(makeCacheKey(crmUserId,"threads"),      threadsCache);  }, [threadsCache,  crmUserId]);
  useEffect(() => { if (crmUserId) save(makeCacheKey(crmUserId,"pageTokens"),   nextPageTokens);}, [nextPageTokens, crmUserId]);
  useEffect(() => { if (crmUserId) save(makeCacheKey(crmUserId,"actualCounts"), actualCounts);  }, [actualCounts,  crmUserId]);
  useEffect(() => { if (crmUserId) save(makeCacheKey(crmUserId,"activeLabel"),  activeLabel);   }, [activeLabel,   crmUserId]);
  useEffect(() => { if (crmUserId) save(makeCacheKey(crmUserId,"lastFetch"),    lastFetchTime); }, [lastFetchTime, crmUserId]);
  useEffect(() => { save(AUTH_KEY,    authStatus);       }, [authStatus]);
  useEffect(() => { save(SIDEBAR_KEY, sidebarCollapsed); }, [sidebarCollapsed]);
  ...
  useEffect(() => {
    if (!crmUserId) return;
    setThreadsCache(load(makeCacheKey(crmUserId,"threads"),      EMPTY_CACHE()));
    setNextPageTokens(load(makeCacheKey(crmUserId,"pageTokens"), {}));
    setActualCounts(load(makeCacheKey(crmUserId,"actualCounts"), EMPTY_COUNTS()));
    setActiveLabel(load(makeCacheKey(crmUserId,"activeLabel"),   "INBOX"));
    setLastFetchTime(load(makeCacheKey(crmUserId,"lastFetch"),   {}));
    setSelectedThread(null); setMessages([]);
    const del = loadSet(makeCacheKey(crmUserId,"deleted"));
    const tr  = loadSet(makeCacheKey(crmUserId,"trashMoved"));
    setDeletedSet(del); deletedRef.current = del;
    setTrashSet(tr);    trashRef.current   = tr;
    setReadSet(new Set()); readRef.current  = new Set();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const doFetchCounts = useCallback(async () => {
    try {
      const res = await secureApi().get("/gmail/all-counts", { timeout: 12000 });
      if (res.data.success) {
        setActualCounts(res.data.counts);
        setLastFetchTime(p => ({ ...p, counts: Date.now() }));
      }
  ...
  const fetchLabel = useCallback(async ({
    ...
    if (!force && !loadMore && label !== "UNREAD" && crmUserId) {
      const cachedData = load(makeCacheKey(crmUserId,"threads"), EMPTY_CACHE());
      const cached     = cachedData[label] || [];
      const times      = load(makeCacheKey(crmUserId,"lastFetch"), {});
      const age        = times[label] ? Date.now() - times[label] : Infinity;
      if (cached.length > 0 && age < 120000) {
        const filtered = cached
          .filter(t => !del.has(t.id))
    ...
    try {
      ...
      if (loadMore && crmUserId) {
        const tokens = load(makeCacheKey(crmUserId,"pageTokens"), {});
        if (tokens[label]) params.pageToken = tokens[label];
      }

      const res = await secureApi().get("/gmail/threads", { params, signal, timeout: 15000 });
  ...
  const fetchDrafts = useCallback(async () => {
    setListLoading(true); setListLoadLabel("DRAFTS");
    try {
      const res = await secureApi().get("/gmail/drafts", { params: { maxResults: 20 } });
      if (res.data.success) {
        const drafts = (res.data.data || [])
          .filter(d => !deletedRef.current.has(d.id))
          .map(d => ({ ...d, isDraft: true }));
  ...
  const bootstrapData = useCallback(async () => {
    ...
    try {
      const res = await secureApi().get("/gmail/labels");
      if (res.data.success) setLabels(res.data.data || []);
    } catch { /* silent */ }
  }, [doFetchCounts, fetchLabel, secureApi]);

  // ─── URL PARAMS (OAuth callback) ──────────────────
  useEffect(() => {
    const connected  = searchParams.get("gmail_connected");
    const gmailError = searchParams.get("gmail_error");
    const email      = searchParams.get("email"); // Gmail address, display only

    if (connected === "1" || connected === "true") {
      toast.success("✅ Gmail connected successfully!");
      navigate("/emailchat", { replace: true });
      if (email) setConnectedEmail(email);
      setAuthStatus({ authenticated: true, message: "Gmail is connected" });
      setActiveLabel("INBOX");
      bootstrapData();
      setIsConnecting(false);
    }
  ...
  useEffect(() => {
    ...
    // If no JWT exists at all, don't even try to check auth — show connect screen
    if (!getAuthToken()) {
      setAuthLoading(false);
      setAuthStatus({ authenticated: false, message: "Please log in to your CRM first" });
      return;
    }

    const cachedAuth = load(AUTH_KEY, { authenticated: false });
    if (cachedAuth.authenticated && crmUserId) {
      setAuthStatus(cachedAuth);
      bootstrapData();
    } else {
      checkAuthStatus();
  ...
  const checkAuthStatus = async () => {
    setAuthLoading(true);
    try {
      const res = await secureApi().get("/gmail/auth-status");
      setAuthStatus(res.data);
      if (res.data.authenticated) {
        if (res.data.email) setConnectedEmail(res.data.email);
        bootstrapData();
      } else {
        await fetchAuthUrl();
      }
    } catch (err) {
      // 401 means the CRM JWT is invalid/expired — user needs to re-login to CRM
      if (err.response?.status === 401) {
        setAuthStatus({ authenticated: false, message: "Your session has expired. Please log in again." });
        setError("Session expired. Please log in to your CRM account first.");
      } else {
        setAuthStatus({ authenticated: false, message: "Error checking authentication" });
        await fetchAuthUrl();
      }
  ...
  const fetchAuthUrl = async () => {
    try {
      const res = await secureApi().get("/gmail/auth-url");
      if (res.data.success) setAuthUrl(res.data.url);
      else setError(res.data.error || "Failed to get authentication URL");
    } catch { setError(`Failed to connect to server on ${SI_URI}`); }
  };
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (activeLabel !== "DRAFTS") {
          ...
          setActualCounts(p => ({ ...p, UNREAD: Math.max(0, p.UNREAD - 1) }));
          secureApi().post(`/gmail/thread/${threadId}/read`, { read: true }).catch(() => {});
        }
        doFetchCounts();
      }
    } catch (err) {
  ...
  }, [activeLabel, secureApi, threadLoading, doFetchCounts]);
  ...
  const handleLabelClick = useCallback(async (labelId) => {
    ...
    if (labelId === "TRASH") {
      setTrashSet(new Set()); trashRef.current = new Set();
      if (crmUserId) localStorage.removeItem(makeCacheKey(crmUserId,"trashMoved"));
    }
  ...
  const markThreadAs = useCallback(async (threadId, action, value = true) => {
    ...
    const bodyMap = { star: { star: value }, important: { important: value }, spam: { spam: value }, trash: {} };
    try {
      const res = await secureApi().post(`/gmail/${endpointMap[action]}`, bodyMap[action]);
  ...
  }, [threadsCache, actualCounts, secureApi, doFetchCounts]);
  ...
  const handleBulkAction = useCallback(async (action, value = true) => {
    ...
    try {
      if (action === "star") {
        setThreadsCache(p => ({ ...p, [activeLabel]: (p[activeLabel]||[]).map(t => selectedThreads.has(t.id) ? { ...t, starred:value } : t) }));
        await secureApi().post("/gmail/bulk-star", { threadIds:ids, star:value });
        toast.success(`⭐ ${value?"Starred":"Unstarred"} ${ids.length} emails`);
      }
      if (action === "trash") {
        setThreadsCache(p => ({ ...p, [activeLabel]: (p[activeLabel]||[]).filter(t => !selectedThreads.has(t.id)) }));
        await secureApi().post("/gmail/bulk-trash", { threadIds:ids });
        toast.success(`Moved ${ids.length} emails to trash`);
      }
      if (action === "delete") {
        setThreadsCache(p => ({ ...p, [activeLabel]: (p[activeLabel]||[]).filter(t => !selectedThreads.has(t.id)) }));
        await secureApi().post("/gmail/bulk-delete", { threadIds:ids, permanent:activeLabel==="TRASH" });
        toast.success(`Deleted ${ids.length} emails`);
      }
      if (action === "read") {
        setReadSet(p => { const s = new Set(p); ids.forEach(id => value ? s.add(id) : s.delete(id)); return s; });
        setThreadsCache(p => ({ ...p, [activeLabel]: (p[activeLabel]||[]).map(t => selectedThreads.has(t.id) ? { ...t, unread:!value } : t) }));
        await Promise.all(ids.map(id => secureApi().post(`/gmail/thread/${id}/read`, { read:value })));
      }
      setSelectedThreads(new Set()); setShowBulkActions(false);
      doFetchCounts();
    } catch { toast.error(`Failed to ${action}`); }
  }, [selectedThreads, activeLabel, secureApi, doFetchCounts]);

  // ─── DELETE THREAD ─────────────────────────────────
  const deleteThreadFn = useCallback(async (threadId, permanent = false) => {
    try {
      if (activeLabel === "DRAFTS") {
        await secureApi().delete(`/gmail/draft/${threadId}`);
        toast.success("📝 Draft deleted");
        setThreadsCache(p => ({ ...p, DRAFTS:(p.DRAFTS||[]).filter(t => t.id !== threadId) }));
        setActualCounts(p => ({ ...p, DRAFTS:Math.max(0, p.DRAFTS-1) }));
        if (selectedThread === threadId) { setSelectedThread(null); setMessages([]); }
      ...
      if (permanent) {
        setDeletedSet(p => { const s = new Set(p); s.add(threadId); return s; });
        await secureApi().delete(`/gmail/thread/${threadId}`);
        toast.success("🗑️ Permanently deleted");
      } else {
        setTrashSet(p => { const s = new Set(p); s.add(threadId); return s; });
        await secureApi().post(`/gmail/thread/${threadId}/trash`);
        toast.success("🗑️ Moved to trash");
      }
      setThreadsCache(p => { const u={...p}; Object.keys(u).forEach(l => { u[l] = u[l].filter(t => t.id !== threadId); }); return u; });
      if (selectedThread === threadId) { setSelectedThread(null); setMessages([]); }
  ...
  }, [activeLabel, secureApi, selectedThread, doFetchCounts]);

  // ─── DISCONNECT ────────────────────────────────────
  const disconnectGmail = async () => {
    setShowDisconnModal(false);
    try {
      await secureApi().delete("/gmail/disconnect");
      if (crmUserId) clearUserStorage(crmUserId);
      setConnectedEmail("");
  ...
  const handleSuggestionFetch = (field, query) => {
    ...
    suggestTimers.current[field] = setTimeout(async () => {
      ...
      if (token.length < 2) { setSuggestions(p => ({ ...p, [field]:[] })); return; }
      try {
        const res = await secureApi().get("/gmail/suggestions", { params:{ query:token } });
        if (res.data.success) setSuggestions(p => ({ ...p, [field]: res.data.data||[] }));
      } catch { setSuggestions(p => ({ ...p, [field]:[] })); }
    }, 400);
  };
  ...
  const openReply = (msg, type = "reply") => {
    ...
    if (type === "reply") {
      ...
      const all = [
        ...
        ...(msg.to||"").split(",").map(e => extractEmail(e.trim())).filter(Boolean),
        ...(msg.cc||"").split(",").map(e => extractEmail(e.trim())).filter(Boolean),
      ].filter((v,i,a) => a.indexOf(v)===i && v!==connectedEmail);
      to      = all.join(", ");
      subject = msg.subject?.startsWith("Re:") ? msg.subject : `Re: ${msg.subject}`;
      message = `\n\nOn ${formatDateTime(msg.date)}, ${extractName(msg.from)} wrote:\n> ${(msg.body||"").substring(0,200)}...`;
    } else {
      ...
      <Dialog open={showDisconnModal} onOpenChange={setShowDisconnModal}>
        <DialogContent className="bg-white rounded-2xl shadow-xl max-w-md">
          ...
          <div className="p-6">
            ...
            <h3 className="text-lg font-semibold text-gray-900 text-center mb-2">Disconnect {connectedEmail}?</h3>
            <p className="text-gray-500 text-center text-sm mb-5">You'll need to reconnect to access your emails again.</p>
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6">
              <p className="text-sm text-yellow-800 flex items-start gap-2">
                <FaInfoCircle size={15} className="flex-shrink-0 mt-0.5" />
          ...
          <div className="space-y-1 mt-4">
            ...
            <div className="border border-gray-200 rounded-xl p-4 bg-gray-50 mt-2">
              <div className="flex items-center justify-between mb-3">
                <label className="text-sm font-medium text-gray-600 flex items-center gap-2">
                  <FaPaperclip size={13} />Attachments
                  <span className="text-gray-400 font-normal text-xs">(max 25 MB{selectedFiles.length > 0 && ` · total ${formatFileSize(totalAttachSize)}`})</span>
                  {totalAttachSize > GMAIL_ATTACH_LIMIT && <span className="text-red-500 text-xs font-semibold">⚠ Over limit!</span>}
                </label>
            ...
            <textarea value={composeData.message} onChange={e => setComposeData(p => ({ ...p, message:e.target.value }))} rows="10"
              className="w-full p-4 border border-gray-200 rounded-xl focus:ring-2 focus:outline-none resize-none text-sm text-gray-800 placeholder-gray-400 mt-2"
              placeholder="Write your message..." />
          </div>
          {sending && sendProgress > 0 && (
      ...
      <Dialog open={showDeleteModal} onOpenChange={setShowDeleteModal}>
        <DialogContent className="bg-white rounded-2xl shadow-xl max-w-md">
          ...
          <div className="p-6">
            ...
            <h3 className="text-lg font-semibold text-gray-900 text-center mb-2">{threadToDelete?.permanent?"Delete permanently?":"Move to trash?"}</h3>
            <p className="text-gray-500 text-sm text-center mb-6">{threadToDelete?.permanent?"This cannot be undone.":"You can restore it from trash later."}</p>
            <div className="flex items-center justify-center gap-3">
              <button onClick={() => { setShowDeleteModal(false); setThreadToDelete(null); }}
                className="px-5 py-2.5 rounded-xl border border-gray-300 hover:bg-gray-50 text-gray-600 text-sm font-medium">Cancel</button>
              <button onClick={() => deleteThreadFn(threadToDelete.id, threadToDelete.permanent)}
      ...
      <div className="flex h-full overflow-hidden">
        ...
        <div className={`${sidebarCollapsed?"w-16":"w-60"} bg-white flex flex-col transition-all duration-300 ease-in-out ${showMobileSidebar?"fixed inset-y-0 left-0 z-40 shadow-xl":"hidden lg:flex"}`}
          ...
          <div className="px-3 py-4 flex items-center justify-between" style={{ borderBottom:"1px solid #e8eaed" }}>
            <div className={`flex items-center ${sidebarCollapsed?"justify-center w-full":"gap-2.5"}`}>
              ...
              {!sidebarCollapsed && (
                <div className="min-w-0">
                  <p className="text-xs font-medium text-gray-800 truncate">{connectedEmail}</p>
                  <p className="text-xs" style={{ color:"#5f6368" }}>{actualCounts.UNREAD} unread</p>
                </div>
              )}
            </div>
          ...
          <div className="flex-1 overflow-y-auto py-2">
            {labelOptions.map(label => {
              ...
              const isActive = activeLabel === label.id;
              return (
                <button key={label.id} onClick={() => handleLabelClick(label.id)}
                  className={`w-full flex items-center ${sidebarCollapsed?"justify-center px-2":"justify-between px-3"} py-2 transition-all duration-100 text-sm`}
        ...
        <div className="flex-1 flex flex-col overflow-hidden" style={{ background:"#f6f8fc" }}>
          <div className="bg-white px-5 py-3 flex-shrink-0" style={{ borderBottom:"1px solid #e8eaed" }}>
            ...
            {showBulkActions && !selectedThread && (
              <div className="mt-3 px-3 py-2 rounded-xl flex items-center gap-3 flex-wrap" style={{ background:"#e8f0fe" }}>
                <span className="text-xs font-medium flex items-center gap-1" style={{ color:"#1a73e8" }}><FaCheckSquare size={13} />{selectedThreads.size} selected</span>
                <button onClick={() => handleBulkAction("read",true)}  className="text-xs bg-white px-3 py-1 rounded-full border" style={{ color:"#1a73e8", borderColor:"#c5d6f5" }}>Mark Read</button>
                <button onClick={() => handleBulkAction("star",true)}  className="text-xs bg-white px-3 py-1 rounded-full border" style={{ color:"#1a73e8", borderColor:"#c5d6f5" }}>⭐ Star</button>
                <button onClick={() => handleBulkAction("trash")}      className="text-xs bg-white px-3 py-1 rounded-full border" style={{ color:"#1a73e8", borderColor:"#c5d6f5" }}>🗑 Trash</button>
            ...
            {!selectedThread && (
              <div className="mt-3 flex gap-2">
                <div className="flex-1 relative">
                  ...
                  <input type="text" value={searchQuery} onChange={e=>setSearchQuery(e.target.value)}
                    placeholder={`Search in ${activeLabel.toLowerCase()}...`}
                    className="w-full pl-9 pr-4 py-2 rounded-2xl text-sm border-0 outline-none" style={{ background:"#eaf1fb", color:"#202124" }} />
                </div>
                {activeLabel === "INBOX" && (
                  <button onClick={() => handleLabelClick("UNREAD")}
                    className="px-3.5 py-2 rounded-2xl text-sm flex items-center gap-2 transition"
                    ...
                    <span className="hidden sm:inline">Unread</span>
                    {actualCounts.UNREAD > 0 && (
                      <span className="text-white text-xs rounded-full px-1.5 py-0.5 font-semibold" style={{ background:"#1a73e8" }}>{actualCounts.UNREAD}</span>
                    )}
                  </button>
                )}
              </div>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {!selectedThread ? (
              <div>
                {listLoading && listLoadLabel===activeLabel && currentThreads.length===0 ? <ThreadSkeleton /> :
                filteredThreads.length === 0 ? (
                  <div className="flex items-center justify-center h-64">
                    <div className="text-center">
                      <div className="w-20 h-20 mx-auto mb-4 rounded-full flex items-center justify-center" style={{ background:"#f1f3f4" }}>
                  ...
                  <div>
                    {messages.map(msg => (
                      <div key={msg.id} className="p-6" style={{ borderBottom:"1px solid #f1f3f4" }}>
                        ...
                        {msg.hasAttachments && msg.attachments?.length > 0 && (
                          <div className="mb-4 p-3 rounded-xl" style={{ background:"#f6f8fc" }}>
                            <h5 className="text-xs font-medium mb-2 flex items-center gap-1.5" style={{ color:"#444746" }}><FaPaperclip size={11} />Attachments ({msg.attachments.length})</h5>
                            <div className="space-y-1.5">
                              {msg.attachments.map((att,idx) => (
                                <div key={idx} className="flex items-center justify-between p-2 bg-white rounded-lg" style={{ border:"1px solid #e8eaed" }}>
                                  <div className="flex items-center gap-2">
                        ...
                        <div className="mt-5 flex items-center gap-2 pt-4" style={{ borderTop:"1px solid #f1f3f4" }}>
                          <button onClick={() => openReply(msg,"reply")} className="text-sm px-4 py-1.5 rounded-full flex items-center gap-1.5" style={{ background:"#c2e7ff", color:"#001d35" }}><FaReply size={13} />Reply</button>
                          <button onClick={() => openReply(msg,"replyAll")} className="text-sm px-4 py-1.5 rounded-full border flex items-center gap-1.5 hover:bg-gray-50" style={{ border:"1px solid #dadce0", color:"#444746" }}><FaReplyAll size={13} />Reply All</button>
                          <button onClick={() => openReply(msg,"forward")} className="text-sm px-4 py-1.5 rounded-full border flex items-center gap-1.5 hover:bg-gray-50" style={{ border:"1px solid #dadce0", color:"#444746" }}><FaForward size={13} />Forward</button>
                        </div>
                      </div>
                    ))}
                  </div>
