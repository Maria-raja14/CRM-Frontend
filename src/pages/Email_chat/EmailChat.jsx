...
/**
 * EmailChat.jsx  (SECURITY-FIXED)
 *
 * Key changes vs old version:
 *  - NEVER sends ?email= or body.email to the API.
 *    The server reads the email from the signed httpOnly session cookie.
 *  - On connect, the email returned in the URL is stored ONLY for display
 *    purposes — it is never used to gate API calls.
 *  - On 401 responses the UI automatically shows the connect screen.
 *  - localStorage still caches threads for UX speed, but the cache is
 *    keyed to the session cookie's email (received from /auth-status),
 *    not a client-supplied param.
 */
