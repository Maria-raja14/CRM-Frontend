...
// ─── STORAGE HELPERS ─────────────────────────────────
// 🔒 Cache is keyed by CRM user ID (from JWT), NOT by Gmail address.
//    This prevents stale data from one CRM user leaking to another.
const makeCacheKey  = (crmUserId, suffix) => crmUserId ? `gmail_${suffix}_${crmUserId}` : null;
const save          = (key, data) => { try { if (key) localStorage.setItem(key, JSON.stringify(data)); } catch { } };
const load          = (key, def)  => { try { if (!key) return def; const s = localStorage.getItem(key); return s ? JSON.parse(s) : def; } catch { return def; } };
const loadSet       = (key)       => { try { if (!key) return new Set(); const s = localStorage.getItem(key); return s ? new Set(JSON.parse(s)) : new Set(); } catch { return new Set(); } };
const clearUserStorage = (crmUserId) => {
  if (!crmUserId) return;
  ["threads","pageTokens","totalCounts","actualCounts","activeLabel","lastFetch","selectedThread","messages","deleted","trashMoved"]
    .forEach(p => { try { localStorage.removeItem(makeCacheKey(crmUserId, p)); } catch { } });
};

// 🔒 Auth state keys include no user-specific data — just auth flags
...
const EmailChat = () => {
  ...
  // ── AUTH ──
  // 🔒 Get JWT token from wherever your app stores it (localStorage key from your auth flow)
  const getAuthToken = () => localStorage.getItem("token") || localStorage.getItem("authToken") || "";

  // 🔒 Get CRM user ID from JWT (decode payload)
  const getCrmUserIdFromToken = () => {
    try {
  ...
  const [authStatus,    setAuthStatus]    = useState(() => load(AUTH_KEY, { authenticated: false, message: "" }));
  const [connectedEmail, setConnectedEmail] = useState(""); // Gmail address for display only
  const [authUrl,       setAuthUrl]       = useState("");
  const [isConnecting,  setIsConnecting]  = useState(false);
  const [error,         setError]         = useState("");
  const [authLoading,   setAuthLoading]   = useState(false);

  // 🔒 CRM user ID — derived from JWT, never from user input
  ...
  // ─── 🔒 SECURE AXIOS INSTANCE — always sends JWT ─────────────────────────
  // Every request to the Gmail API automatically includes the Bearer token.
  // No email param is ever needed — the server reads crm_user_id from the JWT.
  ...
  const fetchLabel = useCallback(async ({
    ...
    try {
      ...
      // 🔒 No email param — server identifies user from JWT
  ...
  useEffect(() => {
    const connected  = searchParams.get("gmail_connected");
    const gmailError = searchParams.get("gmail_error");
    const email      = searchParams.get("email"); // Gmail address for display only
  ...
  const checkAuthStatus = async () => {
    setAuthLoading(true);
    try {
      // 🔒 No email query param — server uses JWT to identify user
  ...
  const fetchAuthUrl = async () => {
    try {
      // 🔒 Server embeds crmUserId in the OAuth state param
  ...
  const markThreadAs = useCallback(async (threadId, action, value = true) => {
    ...
    const bodyMap = {
      star: { star: value }, important: { important: value }, spam: { spam: value }, trash: {},
    };
    try {
  ...
  // ─── DISCONNECT ────────────────────────────────────
  // 🔒 Only disconnects THIS CRM user's Gmail — backend enforces this too
  const disconnectGmail = async () => {
    setShowDisconnModal(false);
    try {
  ...
  const sendEmail = async () => {
    ...
    try {
      ...
      fd.append("subject", composeData.subject?.trim()||"(No Subject)");
      fd.append("message", composeData.message?.trim()||"");
      // 🔒 No email param — server reads sender from the JWT token's DB record
      selectedFiles.forEach(f => fd.append("attachments", f));
