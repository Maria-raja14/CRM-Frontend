...
// ─── JWT HELPER ───────────────────────────────────────
// ✅ SECURITY FIX: Every API call must include the JWT so the server
//    can verify WHICH CRM user is making the request and only return
//    data belonging to that user.
const getAuthToken = () => {
  // Adjust the key name to match where YOUR app stores the JWT.
  // Common keys: "token", "authToken", "jwt", "access_token"
...
const EmailChat = () => {
  ...
  const doFetchCounts = useCallback(async (emailArg) => {
    ...
    if (!email) return;
    try {
      // ✅ SECURITY FIX: send JWT so server verifies ownership
      const res = await axios.get(`${API_BASE}/gmail/all-counts`, {
        params: { email },
        headers: authHeaders(),
        timeout: 12000,
      });
  ...
  const fetchLabel = useCallback(async ({
    ...
    try {
      ...
      // ✅ SECURITY FIX: send JWT
      const res = await axios.get(`${API_BASE}/gmail/threads`, {
        params,
        headers: authHeaders(),
        signal,
        timeout: 15000,
  ...
  const fetchDrafts = useCallback(async (emailArg) => {
    ...
    try {
      // ✅ SECURITY FIX: send JWT
      const res = await axios.get(`${API_BASE}/gmail/drafts`, {
        params: { maxResults: 20, email },
        headers: authHeaders(),
      });
      if (res.data.success) {
        const drafts = (res.data.data || [])
  ...
  const bootstrapForEmail = useCallback(async (email) => {
    ...
    try {
      // ✅ SECURITY FIX: send JWT
      const res = await axios.get(`${API_BASE}/gmail/labels`, {
        params: { email },
        headers: authHeaders(),
      });
      if (res.data.success) setLabels(res.data.data || []);
  ...
  useEffect(() => {
    ...
    if (connected === "1" || connected === "true") {
      ...
      if (returnedEmail) {
        // ✅ SECURITY FIX: After OAuth, link the Gmail account to this CRM user.
        //    Without this, the GmailToken has no crm_user_id and resolveEmail()
        //    on the server will reject all requests with 403.
        axios.post(
          `${API_BASE}/gmail/link-account`,
          { email: returnedEmail },
          { headers: authHeaders() }
        ).then(() => {
          console.log("✅ Gmail account linked to CRM user");
        }).catch(err => {
          console.error("❌ Failed to link Gmail account:", err.message);
          toast.error("Failed to link Gmail account. Please try reconnecting.");
        });
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (activeLabel !== "DRAFTS") {
          ...
          // ✅ SECURITY FIX: send JWT
          axios.post(
            `${API_BASE}/gmail/thread/${threadId}/read`,
            { read: true, email: userEmail },
            { headers: authHeaders() }
          ).catch(() => {});
        }
        doFetchCounts(userEmail);
  ...
  const markThreadAs = useCallback(async (threadId, action, value = true) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${API_BASE}/gmail/${endpointMap[action]}`,
        { email: userEmail, ...bodyMap[action] },
        { headers: authHeaders() }
      );
      if (res.data.success) { toast.success(res.data.message); doFetchCounts(userEmail); }
      else throw new Error(res.data.error);
  ...
  const handleBulkAction = useCallback(async (action, value = true) => {
    ...
    try {
      if (action === "star") {
        setThreadsCache(p => ({ ...p, [activeLabel]: (p[activeLabel]||[]).map(t => selectedThreads.has(t.id) ? { ...t, starred:value } : t) }));
        // ✅ SECURITY FIX: send JWT
        await axios.post(`${API_BASE}/gmail/bulk-star`, { threadIds:ids, star:value, email:userEmail }, { headers: authHeaders() });
        toast.success(`⭐ ${value?"Starred":"Unstarred"} ${ids.length} emails`);
      }
      if (action === "trash") {
        setThreadsCache(p => ({ ...p, [activeLabel]: (p[activeLabel]||[]).filter(t => !selectedThreads.has(t.id)) }));
        await axios.post(`${API_BASE}/gmail/bulk-trash`, { threadIds:ids, email:userEmail }, { headers: authHeaders() });
        toast.success(`Moved ${ids.length} emails to trash`);
      }
      if (action === "delete") {
        setThreadsCache(p => ({ ...p, [activeLabel]: (p[activeLabel]||[]).filter(t => !selectedThreads.has(t.id)) }));
        await axios.post(`${API_BASE}/gmail/bulk-delete`, { threadIds:ids, permanent:activeLabel==="TRASH", email:userEmail }, { headers: authHeaders() });
        toast.success(`Deleted ${ids.length} emails`);
      }
      if (action === "read") {
        setReadSet(p => { const s = new Set(p); ids.forEach(id => value ? s.add(id) : s.delete(id)); return s; });
        ...
        await Promise.all(ids.map(id =>
          axios.post(`${API_BASE}/gmail/thread/${id}/read`, { read:value, email:userEmail }, { headers: authHeaders() })
        ));
      }
      setSelectedThreads(new Set()); setShowBulkActions(false);
      doFetchCounts(userEmail);
  ...
  const deleteThreadFn = useCallback(async (threadId, permanent = false) => {
    try {
      if (activeLabel === "DRAFTS") {
        // ✅ SECURITY FIX: send JWT
        await axios.delete(`${API_BASE}/gmail/draft/${threadId}`, {
      ...
      if (permanent) {
        setDeletedSet(p => { const s = new Set(p); s.add(threadId); return s; });
        await axios.delete(`${API_BASE}/gmail/thread/${threadId}`, {
          data: { email:userEmail },
          headers: authHeaders(),
        });
        toast.success("🗑️ Permanently deleted");
        ...
        setTrashSet(p => { const s = new Set(p); s.add(threadId); return s; });
        await axios.post(`${API_BASE}/gmail/thread/${threadId}/trash`, { email:userEmail }, { headers: authHeaders() });
        toast.success("🗑️ Moved to trash");
      }
      setThreadsCache(p => { const u = {...p}; Object.keys(u).forEach(l => { u[l] = u[l].filter(t => t.id !== threadId); }); return u; });
      if (selectedThread === threadId) { setSelectedThread(null); setMessages([]); }
  ...
  const handleSuggestionFetch = (field, query) => {
    ...
    suggestTimers.current[field] = setTimeout(async () => {
      ...
      try {
        // ✅ SECURITY FIX: send JWT
        const res = await axios.get(`${API_BASE}/gmail/suggestions`, {
          params: { query:token, email:userEmail },
          headers: authHeaders(),
        });
        if (res.data.success) setSuggestions(p => ({ ...p, [field]: res.data.data||[] }));
      } catch { setSuggestions(p => ({ ...p, [field]:[] })); }
