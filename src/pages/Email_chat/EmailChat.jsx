...
// ─────────────────────────────────────────────────────────────────────────────
// JWT HELPER
// Reads the JWT that your existing CRM auth stores in localStorage.
// Adjust the key name if your CRM uses a different one.
// ─────────────────────────────────────────────────────────────────────────────
function getJwtToken() {
  // Common keys – adjust to match what your CRM login stores
  return (
...
// ─────────────────────────────────────────────────────────────────────────────
// AXIOS INSTANCE – always sends Authorization: Bearer <jwt>
// No email is ever sent in query params or request body from the client.
// The server resolves the correct Gmail account from the JWT identity.
// ─────────────────────────────────────────────────────────────────────────────
const API_BASE = import.meta.env.VITE_API_URL || "http://localhost:5000/api";
...
// ─── STORAGE HELPERS ─────────────────────────────────
// Cache is scoped to the CRM user's JWT sub claim (or a hash of the token)
// so two different CRM users on the same browser never share cache.
function getCachePrefix() {
  const token = getJwtToken();
  if (!token) return "gmail_nouser";
  // Use last 16 chars of token as a lightweight user fingerprint
  return `gmail_${token.slice(-16)}`;
}
...
function makeKey(suffix) { return `${getCachePrefix()}_${suffix}`; }
...
const EmailChat = () => {
  ...
  const [userEmail,     setUserEmail]     = useState(""); // display only, never sent to server
  const [authUrl,       setAuthUrl]       = useState("");
  const [isConnecting,  setIsConnecting]  = useState(false);
  const [error,         setError]         = useState("");
  const [authLoading,   setAuthLoading]   = useState(false);
  ...
  useEffect(() => {
    ...
    save(makeKey("deleted"), [...deletedSet]);
  }, [deletedSet]);

  useEffect(() => {
    trashRef.current = trashSet;
    save(makeKey("trashMoved"), [...trashSet]);
  }, [trashSet]);

  useEffect(() => { readRef.current = readSet; }, [readSet]);

  // ── PERSIST UI CACHE ────────────────────────────────
  useEffect(() => { save(makeKey("threads"),      threadsCache);  }, [threadsCache]);
  useEffect(() => { save(makeKey("pageTokens"),   nextPageTokens); }, [nextPageTokens]);
  useEffect(() => { save(makeKey("actualCounts"), actualCounts);  }, [actualCounts]);
  useEffect(() => { save(makeKey("activeLabel"),  activeLabel);   }, [activeLabel]);
  useEffect(() => { save(makeKey("lastFetch"),    lastFetchTime); }, [lastFetchTime]);
  useEffect(() => { save(makeKey("sidebar"),      sidebarCollapsed); }, [sidebarCollapsed]);

  // ── LOAD CACHE ON MOUNT (scoped to current JWT user) ─
  useEffect(() => {
    if (!getJwtToken()) return;
    setThreadsCache(load(makeKey("threads"),      EMPTY_CACHE()));
    setNextPageTokens(load(makeKey("pageTokens"), {}));
    setActualCounts(load(makeKey("actualCounts"), EMPTY_COUNTS()));
    setActiveLabel(load(makeKey("activeLabel"),   "INBOX"));
    setLastFetchTime(load(makeKey("lastFetch"),   {}));
    setSidebarCollapsed(load(makeKey("sidebar"),  false));
    const del = loadSet(makeKey("deleted"));
    const tr  = loadSet(makeKey("trashMoved"));
    setDeletedSet(del); deletedRef.current = del;
    setTrashSet(tr);    trashRef.current   = tr;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // API HELPERS – all calls go through gmailAxios() which injects the JWT.
  // NO email is ever included in request params/body; the server resolves it
  // from the JWT.
  // ─────────────────────────────────────────────────────────────────────────────
  ...
  const doFetchCounts = useCallback(async () => {
    if (!getJwtToken()) return;
    try {
      const res = await gmailAxios().get("/all-counts", { timeout: 12000 });
      if (res.data.success) {
        setActualCounts(res.data.counts);
        setLastFetchTime(p => ({ ...p, counts: Date.now() }));
      }
  ...
  const fetchLabel = useCallback(async ({
    ...
    if (!force && !loadMore && label !== "UNREAD") {
      const cached = load(makeKey("threads"), EMPTY_CACHE());
      const data   = cached[label] || [];
      const times  = load(makeKey("lastFetch"), {});
      const age    = times[label] ? Date.now() - times[label] : Infinity;
      if (data.length > 0 && age < 120000) {
        const filtered = data
          .filter(t => !del.has(t.id))
          .filter(t => label === "TRASH" ? true : !moved.has(t.id))
          .map(t => ({ ...t, unread: readRef.current.has(t.id) ? false : t.unread }));
    ...
    try {
      ...
      if (loadMore) {
        const tokens = load(makeKey("pageTokens"), {});
        if (tokens[label]) params.pageToken = tokens[label];
      }

      const res = await gmailAxios().get("/threads", { params, signal, timeout: 15000 });
  ...
  const fetchDrafts = useCallback(async () => {
    ...
    setListLoading(true); setListLoadLabel("DRAFTS");
    try {
      const res = await gmailAxios().get("/drafts", { params: { maxResults: 20 } });
      if (res.data.success) {
        const drafts = (res.data.data || [])
          .filter(d => !deletedRef.current.has(d.id))
          .map(d => ({ ...d, isDraft: true }));
      ...
      setListLoading(false); setListLoadLabel(null);
    }
  }, []);
  ...
  const bootstrapInbox = useCallback(async () => {
    ...
    await Promise.all([
      doFetchCounts(),
      fetchLabel({ label: "INBOX", force: true }),
    ]);
    const rest = ["UNREAD", "STARRED", "IMPORTANT", "SENT", "SPAM", "TRASH", "DRAFTS"];
    rest.forEach((lbl, i) => {
    ...
    try {
      const res = await gmailAxios().get("/labels");
      if (res.data.success) setLabels(res.data.data || []);
    } catch { /* silent */ }
  }, [doFetchCounts, fetchLabel]);
  ...
  useEffect(() => {
    ...
    if (connected === "1" || connected === "true") {
      toast.success("✅ Gmail connected successfully!");
      navigate("/emailchat", { replace: true });
      if (email) setUserEmail(email); // display only
      setAuthStatus({ authenticated: true, message: "Gmail is connected", email });
      setActiveLabel("INBOX");
      bootstrapInbox();
      setIsConnecting(false);
    }
  ...
  const checkAuthStatus = async () => {
    ...
    try {
      ...
      const res = await gmailAxios().get("/auth-status");
      setAuthStatus(res.data);
      ...
      if (err.response?.status === 401) {
        setAuthStatus({ authenticated: false, message: "CRM session expired. Please log in again." });
      } else {
        setAuthStatus({ authenticated: false, message: "Error checking authentication" });
        await fetchAuthUrl();
      }
  ...
  const fetchAuthUrl = async () => {
    try {
      if (!getJwtToken()) return;
      const res = await gmailAxios().get("/auth-url");
      if (res.data.success) setAuthUrl(res.data.url);
      else setError(res.data.error || "Failed to get authentication URL");
    } catch (err) {
      if (err.response?.status === 401) {
        setError("Please log in to the CRM first.");
      } else {
  ...
  const loadThread = useCallback(async (threadId) => {
    ...
    try {
      ...
      const res = await gmailAxios().get(url, {
        signal: abortThreadRef.current.signal,
        timeout: 15000,
      });
      ...
      if (res.data.success) {
        ...
        if (activeLabel !== "DRAFTS") {
          ...
          setActualCounts(p => ({ ...p, UNREAD: Math.max(0, p.UNREAD - 1) }));
          gmailAxios().post(`/thread/${threadId}/read`, { read: true }).catch(() => { });
        }
        doFetchCounts();
      }
    } catch (err) {
  ...
  }, [activeLabel, threadLoading, doFetchCounts]);
  ...
  const handleLabelClick = useCallback(async (labelId) => {
    ...
    setShowMobileSidebar(false);
  }, [threadsCache, lastFetchTime, fetchLabel, fetchDrafts]);
  ...
  const markThreadAs = useCallback(async (threadId, action, value = true) => {
    ...
    try {
      const res = await gmailAxios().post(endpointMap[action], bodyMap[action]);
  ...
  }, [threadsCache, actualCounts, doFetchCounts]);
  ...
  const handleBulkAction = useCallback(async (action, value = true) => {
    ...
    try {
      if (action === "star") {
        setThreadsCache(p => ({
          ...p,
          [activeLabel]: (p[activeLabel] || []).map(t => selectedThreads.has(t.id) ? { ...t, starred: value } : t),
        }));
        await gmailAxios().post("/bulk-star", { threadIds: ids, star: value });
        toast.success(`⭐ ${value ? "Starred" : "Unstarred"} ${ids.length} emails`);
      }
      if (action === "trash") {
        setThreadsCache(p => ({
        ...
        await gmailAxios().post("/bulk-trash", { threadIds: ids });
        toast.success(`Moved ${ids.length} emails to trash`);
      }
      if (action === "delete") {
        setThreadsCache(p => ({
        ...
        await gmailAxios().post("/bulk-delete", { threadIds: ids, permanent: activeLabel === "TRASH" });
        toast.success(`Deleted ${ids.length} emails`);
      }
      if (action === "read") {
        setReadSet(p => {
        ...
        setThreadsCache(p => ({
          ...p,
          [activeLabel]: (p[activeLabel] || []).map(t => selectedThreads.has(t.id) ? { ...t, unread: !value } : t),
        }));
        await Promise.all(ids.map(id => gmailAxios().post(`/thread/${id}/read`, { read: value })));
      }
      setSelectedThreads(new Set()); setShowBulkActions(false);
      doFetchCounts();
    } catch { toast.error(`Failed to ${action}`); }
  }, [selectedThreads, activeLabel, doFetchCounts]);

  // ── DELETE THREAD ────────────────────────────────────
  const deleteThreadFn = useCallback(async (threadId, permanent = false) => {
    try {
      if (activeLabel === "DRAFTS") {
        await gmailAxios().delete(`/draft/${threadId}`);
        toast.success("📝 Draft deleted");
        setThreadsCache(p => ({ ...p, DRAFTS: (p.DRAFTS || []).filter(t => t.id !== threadId) }));
        setActualCounts(p => ({ ...p, DRAFTS: Math.max(0, p.DRAFTS - 1) }));
        if (selectedThread === threadId) { setSelectedThread(null); setMessages([]); }
      ...
      if (permanent) {
        setDeletedSet(p => { const s = new Set(p); s.add(threadId); return s; });
        await gmailAxios().delete(`/thread/${threadId}`);
        toast.success("🗑️ Permanently deleted");
      } else {
        setTrashSet(p => { const s = new Set(p); s.add(threadId); return s; });
        await gmailAxios().post(`/thread/${threadId}/trash`);
        toast.success("🗑️ Moved to trash");
      }
      setThreadsCache(p => {
        const u = { ...p };
      ...
      setShowDeleteModal(false); setThreadToDelete(null);
    }
  }, [activeLabel, selectedThread, doFetchCounts]);

  // ── DISCONNECT ───────────────────────────────────────
  // Calls DELETE /disconnect with JWT – server only disconnects THIS user's Gmail.
  const disconnectGmail = async () => {
    setShowDisconnModal(false);
    try {
      await gmailAxios().delete("/disconnect");
      clearMyStorage();
      setUserEmail("");
  ...
  const handleSuggestionFetch = (field, query) => {
    ...
    suggestTimers.current[field] = setTimeout(async () => {
      ...
      if (token.length < 2) { setSuggestions(p => ({ ...p, [field]: [] })); return; }
      try {
        const res = await gmailAxios().get("/suggestions", { params: { query: token } });
        if (res.data.success) setSuggestions(p => ({ ...p, [field]: res.data.data || [] }));
      } catch { setSuggestions(p => ({ ...p, [field]: [] })); }
    }, 400);
  };
  ...
  const sendEmail = async () => {
    ...
    try {
      ...
      // Use a one-off axios call that sets both JWT and multipart headers
      const token = getJwtToken();
      const res = await axios.post(`${API_BASE}/gmail/send`, fd, {
        headers: {
