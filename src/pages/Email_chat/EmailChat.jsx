...
/**
 * EmailChat.jsx  —  MULTI-USER ISOLATED VERSION
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * SECURITY MODEL
 * ═══════════════════════════════════════════════════════════════════════════
 *  • Every API call sends the CRM JWT in the Authorization header.
 *  • The server extracts crmUserId from the JWT — the client never sends
 *    an email or userId to select whose mailbox to use.
 *  • `userEmail` state is for DISPLAY only (sidebar avatar, cache namespace).
 *    It is never sent to the server for any auth/lookup purpose.
 *  • Cache is namespaced by email so multiple CRM users on the same browser
 *    tab don't see each other's cached data.
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * TOKEN KEY AUTO-DETECTION
 * ═══════════════════════════════════════════════════════════════════════════
 *  Set VITE_TOKEN_KEY in your frontend .env to match whatever key your
 *  login system uses (e.g. VITE_TOKEN_KEY=token).
 *  Falls back to trying common key names automatically.
 */
...
// ─── JWT TOKEN READER ─────────────────────────────────────────────────────────
// Tries VITE_TOKEN_KEY first, then common key names.
// Set VITE_TOKEN_KEY=<your_key> in .env to match your login system.
const TOKEN_KEYS = [
...
function getAuthToken() {
  for (const key of TOKEN_KEYS) {
    ...
    if (val && val.length > 10) return val; // basic sanity check
  }
  return null;
}
...
// The server resolves the user from the token — we never send email to identify user.
  ...
  instance.interceptors.response.use(
    ...
    (err) => {
      if (err.response?.status === 401 && import.meta.env.DEV) {
        console.warn(
          "[EmailChat] 401 Unauthorized — JWT not found or invalid.\n" +
          "Tried localStorage/sessionStorage keys:", TOKEN_KEYS, "\n" +
          "Current localStorage keys:", Object.keys(localStorage), "\n" +
          "Fix: Add VITE_TOKEN_KEY=<your_key> to your .env file"
        );
      }
...
// ─── STORAGE HELPERS ──────────────────────────────────────────────────────────
// Cache namespaced by email so two CRM users on the same browser don't share cache
const makeKey = (email, suffix) => email ? `gmail_${suffix}_${email}` : null;
const save    = (key, val) => { try { if (key) localStorage.setItem(key, JSON.stringify(val)); } catch { } };
const load    = (key, def) => { try { if (!key) return def; const s = localStorage.getItem(key); return s ? JSON.parse(s) : def; } catch { return def; } };
const loadSet = (key)      => { try { if (!key) return new Set(); const s = localStorage.getItem(key); return s ? new Set(JSON.parse(s)) : new Set(); } catch { return new Set(); } };
...
const clearUserStorage = (email) => {
  if (!email) return;
  ["threads","pageTokens","totalCounts","actualCounts","activeLabel","lastFetch","selectedThread","messages","deleted","trashMoved"]
    .forEach((p) => { try { localStorage.removeItem(makeKey(email, p)); } catch { } });
};

const AUTH_KEY    = "gmail_auth_status_v4";
    ...
    <div className="min-h-screen w-full flex" style={{ fontFamily: "Roboto, Arial, sans-serif" }}>
      ...
      <div className="hidden lg:flex lg:w-1/2 xl:w-3/5 flex-col relative overflow-hidden"
        ...
        <div className="relative z-10 flex flex-col h-full p-12 lg:p-16">
          ...
          <p className="text-blue-300 text-xs">Each user's Gmail is connected privately and securely.</p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 xl:w-2/5 flex items-center justify-center bg-white p-8 md:p-12 lg:p-16">
        <div className="w-full max-w-sm">
          ...
          <p className="text-gray-500 mb-7">Link your personal Gmail account</p>
          ...
          <div className="mt-7 p-4 rounded-xl border border-gray-100 bg-gray-50 flex items-start gap-3">
            ...
            <div>
              <p className="text-gray-700 text-sm font-medium mb-0.5">Private & Isolated</p>
              <p className="text-gray-400 text-xs leading-relaxed">
                Your Gmail is connected only to your account. Other users cannot see your emails.
              </p>
            </div>
          </div>
        </div>
...
const MessageSkeleton = () => (
  <div className="animate-pulse p-6">
    ...
    {[100,90,80,95,75,85].map((w, i) => <div key={i} className="h-3 bg-gray-100 rounded mb-2.5" style={{ width:`${w}%` }} />)}
  </div>
);
...
const EmailChat = () => {
  ...
  // Single stable axios instance with JWT interceptor
  const api = useRef(createApiInstance(`${API_BASE}/gmail`)).current;
  ...
  useEffect(() => {
    ...
    if (userEmail) save(makeKey(userEmail,"deleted"), [...deletedSet]);
  }, [deletedSet, userEmail]);
  useEffect(() => {
    trashRef.current = trashSet;
    if (userEmail) save(makeKey(userEmail,"trashMoved"), [...trashSet]);
  }, [trashSet, userEmail]);
  useEffect(() => { readRef.current = readSet; }, [readSet]);

  // ── PERSIST STATE ─────────────────────────────────────────────────────────
  useEffect(() => { if (userEmail) save(makeKey(userEmail,"threads"),      threadsCache);  }, [threadsCache, userEmail]);
  useEffect(() => { if (userEmail) save(makeKey(userEmail,"pageTokens"),   nextPageTokens);}, [nextPageTokens, userEmail]);
  useEffect(() => { if (userEmail) save(makeKey(userEmail,"actualCounts"), actualCounts);  }, [actualCounts, userEmail]);
  useEffect(() => { if (userEmail) save(makeKey(userEmail,"activeLabel"),  activeLabel);   }, [activeLabel, userEmail]);
  useEffect(() => { if (userEmail) save(makeKey(userEmail,"lastFetch"),    lastFetchTime); }, [lastFetchTime, userEmail]);
  useEffect(() => { save(AUTH_KEY,    authStatus);       }, [authStatus]);
  useEffect(() => { save(EMAIL_KEY,   userEmail);        }, [userEmail]);
  useEffect(() => { save(SIDEBAR_KEY, sidebarCollapsed); }, [sidebarCollapsed]);

  // ── LOAD CACHE WHEN EMAIL CHANGES ─────────────────────────────────────────
  useEffect(() => {
    if (!userEmail) return;
    const e = userEmail;
    setThreadsCache(load(makeKey(e,"threads"),      EMPTY_CACHE()));
    setNextPageTokens(load(makeKey(e,"pageTokens"), {}));
    setActualCounts(load(makeKey(e,"actualCounts"), EMPTY_COUNTS()));
    setActiveLabel(load(makeKey(e,"activeLabel"),   "INBOX"));
    setLastFetchTime(load(makeKey(e,"lastFetch"),   {}));
    setSelectedThread(null); setMessages([]);
    const del = loadSet(makeKey(e,"deleted"));
    const tr  = loadSet(makeKey(e,"trashMoved"));
    setDeletedSet(del); deletedRef.current = del;
    setTrashSet(tr);    trashRef.current   = tr;
    setReadSet(new Set()); readRef.current = new Set();
  }, [userEmail]);
  ...
  const doFetchCounts = useCallback(async () => {
    try {
      const res = await api.get("/all-counts", { timeout: 12000 });
      if (res.data.success) setActualCounts(res.data.counts);
    } catch { /* silent — count failure shouldn't break the UI */ }
  }, [api]);

  // ── FETCH LABEL ───────────────────────────────────────────────────────────
  const fetchLabel = useCallback(async ({ label = "INBOX", loadMore = false, force = false, background = false }) => {
    ...
    // Use cache if fresh
    if (!force && !loadMore && label !== "UNREAD") {
      const cachedData = load(makeKey(userEmail,"threads"), EMPTY_CACHE());
      const cached     = cachedData[label] || [];
      const times      = load(makeKey(userEmail,"lastFetch"), {});
      const age        = times[label] ? Date.now() - times[label] : Infinity;
      if (cached.length > 0 && age < 120000) {
        const filtered = cached
          .filter((t) => !del.has(t.id))
    ...
    try {
      ...
      if (loadMore) {
        const tokens = load(makeKey(userEmail,"pageTokens"), {});
        if (tokens[label]) params.pageToken = tokens[label];
      }
      const res = await api.get("/threads", { params, signal, timeout: 15000 });
      if (res.data.success) {
        const threads = (res.data.data || [])
          .filter((t) => !del.has(t.id))
          .filter((t) => label !== "TRASH" ? !moved.has(t.id) : true)
          .map((t) => ({ ...t, unread: readRef.current.has(t.id) ? false : t.unread }))
          .sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
        setThreadsCache((p) => ({ ...p, [label]: loadMore ? [...(p[label]||[]), ...threads] : threads }));
        setNextPageTokens((p) => ({ ...p, [label]: res.data.nextPageToken }));
        setLastFetchTime((p) => ({ ...p, [label]: Date.now() }));
        if (!loadMore) { setSelectedThreads(new Set()); setShowBulkActions(false); }
      }
  ...
  }, [api, authStatus.authenticated, userEmail]);
  ...
  const fetchDrafts = useCallback(async () => {
    ...
    setListLoading(true); setListLoadLabel("DRAFTS");
    try {
      const res = await api.get("/drafts", { params: { maxResults: 20 } });
      if (res.data.success) {
        const drafts = (res.data.data||[])
          .filter((d) => !deletedRef.current.has(d.id))
          .map((d) => ({ ...d, isDraft: true }));
  ...
  const bootstrapForEmail = useCallback(async (email) => {
    ...
    ["UNREAD","STARRED","IMPORTANT","SENT","SPAM","TRASH","DRAFTS"].forEach((lbl, i) => {
  ...
  const checkAuthStatus = async () => {
    ...
    try {
      ...
      if (err.response?.status === 401) {
        setError("Your CRM session has expired. Please log in again.");
        setAuthStatus({ authenticated: false, message: "Session expired" });
      } else {
  ...
  const fetchAuthUrl = async () => {
    try {
      ...
      if (res.data.success) setAuthUrl(res.data.url);
      else setError(res.data.error || "Failed to get auth URL");
    } catch (err) {
      if (err.response?.status === 401)
        setError("Please log in to the CRM first, then connect Gmail.");
      else
        setError(`Cannot reach server at ${SI_URI}. Please check your connection.`);
    }
  };
  ...
  const loadThread = useCallback(async (threadId) => {
    ...
    setSelectedThread(threadId); setMessages([]); setThreadLoading(true); setError("");
    try {
      const url = activeLabel === "DRAFTS" ? `/draft/${threadId}` : `/thread/${threadId}`;
      const res = await api.get(url, { signal: abortThreadRef.current.signal, timeout: 15000 });
      if (res.data.success) {
        ...
        if (activeLabel !== "DRAFTS") {
          ...
          setActualCounts((p) => ({ ...p, UNREAD: Math.max(0, p.UNREAD-1) }));
          api.post(`/thread/${threadId}/read`, { read: true }).catch(() => {});
        }
        doFetchCounts();
      }
    } catch (err) {
  ...
  }, [activeLabel, api, threadLoading, doFetchCounts]);

  // ── LABEL NAVIGATION ──────────────────────────────────────────────────────
  const handleLabelClick = useCallback(async (labelId) => {
    if (abortListRef.current) abortListRef.current.abort();
    setActiveLabel(labelId); setSelectedThread(null); setMessages([]);
    openingRef.current = null;
    setSearchQuery(""); setSelectedThreads(new Set()); setShowBulkActions(false);
    if (labelId === "TRASH") {
      setTrashSet(new Set()); trashRef.current = new Set();
      if (userEmail) localStorage.removeItem(makeKey(userEmail,"trashMoved"));
    }
    if (labelId === "DRAFTS") {
      const cached = threadsCache.DRAFTS || [];
      const age    = lastFetchTime.DRAFTS ? Date.now() - lastFetchTime.DRAFTS : Infinity;
      ...
      const cached = threadsCache[labelId] || [];
      const age    = lastFetchTime[labelId] ? Date.now() - lastFetchTime[labelId] : Infinity;
      await fetchLabel({ label: labelId, force: labelId==="UNREAD" || cached.length===0 || age>=120000 });
    }
    setShowMobileSidebar(false);
  }, [userEmail, threadsCache, lastFetchTime, fetchLabel, fetchDrafts]);
  ...
  const markThreadAs = useCallback(async (threadId, action, value = true) => {
    ...
    if (action==="star") setActualCounts((p) => ({ ...p, STARRED: value ? p.STARRED+1 : Math.max(0,p.STARRED-1) }));
    const ep = { star:`/thread/${threadId}/star`, important:`/thread/${threadId}/important`, spam:`/thread/${threadId}/spam`, trash:`/thread/${threadId}/trash` };
    const bd = { star:{star:value}, important:{important:value}, spam:{spam:value}, trash:{} };
    try {
      await api.post(ep[action], bd[action]);
      toast.success(`${action.charAt(0).toUpperCase()+action.slice(1)} updated`);
      doFetchCounts();
    } catch {
      setThreadsCache(prevCache); setActualCounts(prevCounts);
      toast.error(`Failed to ${action} thread`);
    }
  }, [threadsCache, actualCounts, api, doFetchCounts]);
  ...
  const handleBulkAction = useCallback(async (action, value = true) => {
    ...
    try {
      if (action==="star") {
        setThreadsCache((p) => ({ ...p, [activeLabel]:(p[activeLabel]||[]).map((t) => selectedThreads.has(t.id) ? {...t,starred:value} : t) }));
        await api.post("/bulk-star", { threadIds:ids, star:value });
        toast.success(`${value?"Starred":"Unstarred"} ${ids.length} emails`);
      }
      if (action==="trash") {
        setThreadsCache((p) => ({ ...p, [activeLabel]:(p[activeLabel]||[]).filter((t) => !selectedThreads.has(t.id)) }));
        await api.post("/bulk-trash", { threadIds:ids });
        toast.success(`Moved ${ids.length} emails to trash`);
      }
      if (action==="delete") {
        setThreadsCache((p) => ({ ...p, [activeLabel]:(p[activeLabel]||[]).filter((t) => !selectedThreads.has(t.id)) }));
        await api.post("/bulk-delete", { threadIds:ids, permanent:activeLabel==="TRASH" });
        toast.success(`Deleted ${ids.length} emails`);
      }
      if (action==="read") {
        setReadSet((p) => { const s=new Set(p); ids.forEach((id) => value?s.add(id):s.delete(id)); return s; });
        setThreadsCache((p) => ({ ...p, [activeLabel]:(p[activeLabel]||[]).map((t) => selectedThreads.has(t.id) ? {...t,unread:!value} : t) }));
        await Promise.all(ids.map((id) => api.post(`/thread/${id}/read`, { read:value })));
      }
      setSelectedThreads(new Set()); setShowBulkActions(false);
      doFetchCounts();
    } catch { toast.error(`Failed to ${action}`); }
  }, [selectedThreads, activeLabel, api, doFetchCounts]);

  // ── DELETE THREAD ─────────────────────────────────────────────────────────
  const deleteThreadFn = useCallback(async (threadId, permanent = false) => {
    try {
      if (activeLabel === "DRAFTS") {
        await api.delete(`/draft/${threadId}`);
        toast.success("Draft deleted");
        setThreadsCache((p) => ({ ...p, DRAFTS:(p.DRAFTS||[]).filter((t) => t.id!==threadId) }));
        setActualCounts((p) => ({ ...p, DRAFTS:Math.max(0,p.DRAFTS-1) }));
        if (selectedThread===threadId) { setSelectedThread(null); setMessages([]); }
      ...
      if (permanent) {
        ...
        setTrashSet((p) => { const s=new Set(p); s.add(threadId); return s; });
        await api.post(`/thread/${threadId}/trash`);
  ...
  }, [activeLabel, api, selectedThread, doFetchCounts]);
  ...
  const handleSuggestionFetch = (field, query) => {
    ...
    suggestTimers.current[field] = setTimeout(async () => {
      ...
      if (token.length < 2) { setSuggestions((p) => ({ ...p, [field]:[] })); return; }
      try {
        const res = await api.get("/suggestions", { params:{ query:token } });
        if (res.data.success) setSuggestions((p) => ({ ...p, [field]:res.data.data||[] }));
      } catch { setSuggestions((p) => ({ ...p, [field]:[] })); }
    }, 400);
  };
  ...
  const openReply = (msg, type = "reply") => {
    ...
    if (type==="reply") {
      to=extractEmail(msg.from);
      subject=msg.subject?.startsWith("Re:")?msg.subject:`Re: ${msg.subject}`;
      message=`\n\nOn ${new Date(msg.date).toLocaleString()}, ${extractName(msg.from)} wrote:\n> ${(msg.body||"").substring(0,200)}...`;
    } else if (type==="replyAll") {
      const all=[extractEmail(msg.from), ...(msg.to||"").split(",").map((e)=>extractEmail(e.trim())).filter(Boolean), ...(msg.cc||"").split(",").map((e)=>extractEmail(e.trim())).filter(Boolean)].filter((v,i,a)=>a.indexOf(v)===i&&v!==userEmail);
      to=all.join(", ");
      subject=msg.subject?.startsWith("Re:")?msg.subject:`Re: ${msg.subject}`;
      message=`\n\nOn ${new Date(msg.date).toLocaleString()}, ${extractName(msg.from)} wrote:\n> ${(msg.body||"").substring(0,200)}...`;
    } else {
  ...
  const sendEmail = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        toast.success("Email sent successfully!");
        clearCompose(); setShowCompose(false); doFetchCounts();
  ...
  const saveAsDraft = async () => {
    ...
    try {
      ...
      selectedFiles.forEach((f) => fd.append("attachments", f));
      const res = await api.post("/draft", fd, { headers:{ "Content-Type":"multipart/form-data" } });
      if (res.data.success) {
        toast.success("Draft saved!"); clearCompose(); setShowCompose(false); doFetchCounts();
  ...
  const downloadAttachment = async (messageId, attachment) => {
    try {
      const res  = await api.get(`/attachment/${messageId}/${attachment.id}`, { responseType:"blob" });
      const url  = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement("a");
      link.href = url; link.setAttribute("download", attachment.filename);
      document.body.appendChild(link); link.click(); link.remove();
  ...
  const extractName  = (s) => { if (!s) return "Unknown"; const m=s.match(/(.*?)</); return m?m[1].trim():s; };
  const extractEmail = (s) => { if (!s) return ""; const m=s.match(/<([^>]+)>/); return m?m[1]:s; };
  const formatSize   = (b) => { if (!b) return "0 B"; const k=1024,arr=["B","KB","MB","GB"],i=Math.min(Math.floor(Math.log(b)/Math.log(k)),3); return `${parseFloat((b/Math.pow(k,i)).toFixed(1))} ${arr[i]}`; };
  const getFileIcon  = (file) => {
    const t=file.type||file.mimeType||"", ext=(file.name||file.filename||"").split(".").pop().toLowerCase();
    if (t.startsWith("image/")||["jpg","jpeg","png","gif","svg","webp"].includes(ext)) return <FaImage className="w-5 h-5 text-blue-500" />;
    if (t.includes("pdf")||ext==="pdf")   return <FaFilePdf className="w-5 h-5 text-red-500" />;
    if (t.includes("audio"))              return <FaFileAudio className="w-5 h-5 text-purple-500" />;
    if (t.includes("video"))              return <FaFileVideo className="w-5 h-5 text-pink-500" />;
    if (["zip","rar","7z","tar","gz"].includes(ext)) return <FaFileArchive className="w-5 h-5 text-yellow-600" />;
    if (["doc","docx","txt"].includes(ext)) return <FaFileAlt className="w-5 h-5 text-blue-700" />;
    if (["xls","xlsx","csv"].includes(ext)) return <FaFileExcel className="w-5 h-5 text-green-600" />;
    if (["js","ts","py","html","css"].includes(ext)) return <FaFileCode className="w-5 h-5 text-green-700" />;
  ...
  const currentThreads = (threadsCache[activeLabel] || [])
    .filter((t) => !deletedRef.current.has(t.id) && (activeLabel==="TRASH" ? true : !trashRef.current.has(t.id)));
  const filteredThreads = currentThreads.filter((t) => {
    if (activeLabel==="UNREAD" && !t.unread) return false;
    const q = searchQuery.toLowerCase();
    return !q || t.subject?.toLowerCase().includes(q) || t.from?.toLowerCase().includes(q) || t.snippet?.toLowerCase().includes(q);
  ...
  const getSidebarCount = (id) => {
    if (id==="TRASH") { const tr=(threadsCache["TRASH"]||[]).filter((t) => !deletedRef.current.has(t.id)); return tr.length>0?actualCounts.TRASH:tr.length; }
    return actualCounts[id] || 0;
  };
  const labelOptions = [
    { id:"INBOX",     name:"Inbox",     icon:<FaInbox size={15} /> },
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden">
        ...
        <DialogContent className="bg-white rounded-2xl shadow-xl max-w-md">
          ...
          <div className="p-6">
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6">
              <p className="text-sm text-yellow-800 flex items-start gap-2">
                <FaInfoCircle size={15} className="flex-shrink-0 mt-0.5" />
                Your emails are safe on Google's servers and won't be deleted.
              </p>
            </div>
            <div className="flex justify-center gap-3">
              <button onClick={() => setShowDisconnModal(false)} className="px-5 py-2.5 rounded-xl border border-gray-300 hover:bg-gray-50 text-gray-600 text-sm font-medium">Cancel</button>
              <button onClick={disconnectGmailFn} className="px-5 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white flex items-center gap-2 text-sm font-medium">
                <FaSignOutAlt size={13} />Disconnect
              </button>
            </div>
          </div>
      ...
      <Dialog open={showCompose} onOpenChange={(open) => { if (!open) { setShowCompose(false); clearCompose(); } else setShowCompose(true); }}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-white rounded-2xl shadow-xl">
          ...
          <div className="space-y-1 mt-4">
            <EmailInputWithSuggestions label="To" field="to" required value={composeData.to} onChange={handleComposeChange} suggestions={suggestions.to} onSelectSuggestion={handleSelectSuggestion} placeholder="Recipient email (comma separated)" />
            <EmailInputWithSuggestions label="Cc" field="cc" value={composeData.cc} onChange={handleComposeChange} suggestions={suggestions.cc} onSelectSuggestion={handleSelectSuggestion} placeholder="Cc (optional)" />
            <EmailInputWithSuggestions label="Bcc" field="bcc" value={composeData.bcc} onChange={handleComposeChange} suggestions={suggestions.bcc} onSelectSuggestion={handleSelectSuggestion} placeholder="Bcc (optional)" />
            <div className="flex items-center border-b border-gray-100 pb-2">
              <label className="w-16 text-sm font-medium text-gray-500 flex-shrink-0">Subject</label>
              <input type="text" value={composeData.subject} onChange={(e) => setComposeData((p) => ({...p,subject:e.target.value}))}
                className="flex-1 p-2 border-none focus:ring-0 focus:outline-none text-gray-800 text-sm placeholder-gray-400" placeholder="Subject (optional)" />
            </div>
            <div className="border border-gray-200 rounded-xl p-4 bg-gray-50 mt-2">
              <div className="flex items-center justify-between mb-3">
                <label className="text-sm font-medium text-gray-600 flex items-center gap-2">
                  <FaPaperclip size={13} />Attachments
                  <span className="text-gray-400 font-normal text-xs">(max 25 MB{selectedFiles.length>0&&` · total ${formatSize(totalAttachSize)}`})</span>
                  {totalAttachSize>GMAIL_ATTACH_LIMIT&&<span className="text-red-500 text-xs font-semibold">⚠ Over limit!</span>}
                </label>
                <button onClick={() => fileInputRef.current?.click()} className="text-xs text-white px-3 py-1.5 rounded-lg flex items-center gap-1.5" style={{background:"#1a73e8"}}>
                  <FaPlus size={10}/>Add
                </button>
                  ...
                  {selectedFiles.map((f,i) => (
                    <div key={i} className={`flex items-center justify-between p-2.5 bg-white rounded-lg border ${f.size>GMAIL_ATTACH_LIMIT?"border-red-300 bg-red-50":"border-gray-100"}`}>
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs font-medium truncate text-gray-700">{f.name}</p>
                          <p className={`text-xs ${f.size>GMAIL_ATTACH_LIMIT?"text-red-500 font-semibold":"text-gray-400"}`}>{formatSize(f.size)}</p>
                        </div>
                      </div>
                      <button onClick={() => setSelectedFiles((p) => p.filter((_,j) => j!==i))} className="ml-2 text-red-400 hover:text-red-600 p-1"><FaTimes size={12}/></button>
                    </div>
            ...
            <textarea value={composeData.message} onChange={(e) => setComposeData((p) => ({...p,message:e.target.value}))}
              rows="10" className="w-full p-4 border border-gray-200 rounded-xl focus:ring-2 focus:outline-none resize-none text-sm text-gray-800 placeholder-gray-400 mt-2"
              placeholder="Write your message..." />
          </div>
          {sending && sendProgress>0 && (
            <div className="mt-4">
              <div className="flex justify-between text-xs text-gray-500 mb-1"><span>Sending...</span><span>{sendProgress}%</span></div>
              <div className="w-full bg-gray-100 rounded-full h-1.5">
                <div className="h-1.5 rounded-full transition-all" style={{width:`${sendProgress}%`,background:"#1a73e8"}} />
              </div>
            </div>
          )}
          <div className="flex justify-between items-center mt-5 pt-4 border-t border-gray-100">
            <div className="flex gap-2">
              <button onClick={clearCompose} className="px-3 py-2 text-gray-500 border border-gray-200 rounded-lg hover:bg-gray-50 flex items-center gap-1.5 text-xs"><FaTrash size={12}/>Clear</button>
              <button onClick={saveAsDraft} disabled={savingDraft||!composeData.to.trim()} className="px-3 py-2 bg-gray-100 text-gray-600 rounded-lg border border-gray-200 hover:bg-gray-200 disabled:opacity-50 flex items-center gap-1.5 text-xs">
                {savingDraft?<><FaSpinner className="animate-spin" size={11}/>Saving...</>:<><FaSave size={12}/>Draft</>}
              </button>
            </div>
            <div className="flex gap-2">
      ...
      <Dialog open={showDeleteModal} onOpenChange={setShowDeleteModal}>
        <DialogContent className="bg-white rounded-2xl shadow-xl max-w-md">
          ...
          <div className="p-6">
            ...
            <div className="flex justify-center gap-3">
              <button onClick={() => { setShowDeleteModal(false); setThreadToDelete(null); }} className="px-5 py-2.5 rounded-xl border border-gray-300 hover:bg-gray-50 text-gray-600 text-sm font-medium">Cancel</button>
              <button onClick={() => deleteThreadFn(threadToDelete.id, threadToDelete.permanent)}
                className={`px-5 py-2.5 rounded-xl text-white flex items-center gap-2 text-sm font-medium ${threadToDelete?.permanent?"bg-red-600 hover:bg-red-700":"bg-orange-500 hover:bg-orange-600"}`}>
                <FaTrash size={13}/>{threadToDelete?.permanent?"Delete":"Move to Trash"}
              </button>
      ...
      {/* LAYOUT */}
      <div className="flex h-full overflow-hidden">
        <button onClick={() => setShowMobileSidebar(!showMobileSidebar)} className="lg:hidden fixed top-4 left-4 z-50 bg-white p-2.5 rounded-xl shadow-lg border border-gray-200">
          <FaBars size={16} color="#5f6368"/>
        </button>

        {/* SIDEBAR */}
        <div className={`${sidebarCollapsed?"w-16":"w-60"} bg-white flex flex-col transition-all duration-300 ${showMobileSidebar?"fixed inset-y-0 left-0 z-40 shadow-xl":"hidden lg:flex"}`}
          style={{borderRight:"1px solid #e8eaed"}}>
          <div className="px-3 py-4 flex items-center justify-between" style={{borderBottom:"1px solid #e8eaed"}}>
            <div className={`flex items-center ${sidebarCollapsed?"justify-center w-full":"gap-2.5"}`}>
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-medium flex-shrink-0"
          ...
          <div className="p-3" style={{borderBottom:"1px solid #f1f3f4"}}>
            <button onClick={() => { setComposeMode("new"); setShowCompose(true); }}
              className={`w-full text-sm font-medium py-2.5 rounded-2xl flex items-center justify-center gap-2 hover:shadow-md ${sidebarCollapsed?"px-2":"px-4"}`}
              style={{background:"#c2e7ff",color:"#001d35"}}>
              <FaEdit size={14} className="flex-shrink-0"/>
              {!sidebarCollapsed&&<span>Compose</span>}
            </button>
          ...
          <div className="flex-1 overflow-y-auto py-2">
            {labelOptions.map((lbl) => {
              const count    = getSidebarCount(lbl.id);
              const isActive = activeLabel===lbl.id;
              return (
                <button key={lbl.id} onClick={() => handleLabelClick(lbl.id)}
                  className={`w-full flex items-center ${sidebarCollapsed?"justify-center px-2":"justify-between px-3"} py-2 text-sm transition-all`}
                  style={{borderRadius:"0 100px 100px 0",background:isActive?"#d3e3fd":"transparent",color:isActive?"#1a73e8":"#444746",fontWeight:isActive?600:400,marginRight:"8px"}}
                  onMouseEnter={(e) => { if(!isActive) e.currentTarget.style.background="#f1f3f4"; }}
                  onMouseLeave={(e) => { e.currentTarget.style.background=isActive?"#d3e3fd":"transparent"; }}
                  title={sidebarCollapsed?lbl.name:""}>
                  <div className={`flex items-center ${sidebarCollapsed?"":"gap-3"}`}>
                    <span style={{color:isActive?"#1a73e8":"#444746"}}>{lbl.icon}</span>
                    {!sidebarCollapsed&&<span>{lbl.name}</span>}
                  </div>
                  {!sidebarCollapsed&&count>0&&(
                    <span className="text-xs font-semibold" style={{color:isActive?"#1a73e8":"#444746"}}>
                      {count>999?`${Math.floor(count/1000)}k`:count}
        ...
        <div className="flex-1 flex flex-col overflow-hidden" style={{background:"#f6f8fc"}}>
          <div className="bg-white px-5 py-3 flex-shrink-0" style={{borderBottom:"1px solid #e8eaed"}}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                {selectedThread&&(
                  <button onClick={() => { setSelectedThread(null); setMessages([]); openingRef.current=null; if(abortThreadRef.current) abortThreadRef.current.abort(); }}
              ...
              {!selectedThread&&(
                <button onClick={() => fetchLabel({label:activeLabel,force:true})} disabled={listLoading&&listLoadLabel===activeLabel}
                  className="p-2 rounded-full hover:bg-gray-100" style={{color:"#5f6368"}}>
                  {listLoading&&listLoadLabel===activeLabel?<FaSpinner className="animate-spin" size={15}/>:<FaSync size={15}/>}
                </button>
              )}
            </div>
            {showBulkActions&&!selectedThread&&(
              <div className="mt-3 px-3 py-2 rounded-xl flex items-center gap-3 flex-wrap" style={{background:"#e8f0fe"}}>
                ...
                <button onClick={()=>handleBulkAction("star",true)}  className="text-xs bg-white px-3 py-1 rounded-full border" style={{color:"#1a73e8",borderColor:"#c5d6f5"}}>⭐ Star</button>
                <button onClick={()=>handleBulkAction("trash")}      className="text-xs bg-white px-3 py-1 rounded-full border" style={{color:"#1a73e8",borderColor:"#c5d6f5"}}>🗑 Trash</button>
                <button onClick={()=>{setSelectedThreads(new Set());setShowBulkActions(false);}} className="ml-auto text-xs" style={{color:"#5f6368"}}>Clear</button>
              </div>
            )}
            {!selectedThread&&(
              <div className="mt-3 flex gap-2">
                ...
                {activeLabel==="INBOX"&&(
                  <button onClick={()=>handleLabelClick("UNREAD")}
                    className="px-3.5 py-2 rounded-2xl text-sm flex items-center gap-2" style={{background:"#fff",border:"1px solid #e8eaed",color:"#5f6368"}}>
                    <FaEnvelope size={13}/>
                    <span className="hidden sm:inline">Unread</span>
                    {actualCounts.UNREAD>0&&<span className="text-white text-xs rounded-full px-1.5 py-0.5 font-semibold" style={{background:"#1a73e8"}}>{actualCounts.UNREAD}</span>}
                  </button>
                )}
              </div>
            )}
                ...
                {listLoading&&listLoadLabel===activeLabel&&currentThreads.length===0 ? (
                  ...
                  <div className="flex items-center justify-center h-64">
                    <div className="text-center">
                      ...
                      <h3 className="text-base font-medium mb-1" style={{color:"#5f6368"}}>No emails found</h3>
                      <p className="text-sm" style={{color:"#80868b"}}>
                        {searchQuery?"Try adjusting your search":activeLabel==="UNREAD"?"You're all caught up! 🎉":`No emails in ${activeLabel.toLowerCase()}`}
                      </p>
                    </div>
                  </div>
                ) : (
                  <div className="rounded-2xl overflow-hidden bg-white" style={{border:"1px solid #e8eaed"}}>
                    {filteredThreads.map((thread) => {
                      ...
                      return (
                        <div key={thread.id}
                          className="group flex items-center px-4 py-2.5 border-b last:border-0 transition-colors"
                          style={{borderColor:"#f1f3f4",background:thread.unread?"#f8f9ff":"white"}}
                          onMouseEnter={(e)=>{e.currentTarget.style.background="#f0f4ff";}}
                          onMouseLeave={(e)=>{e.currentTarget.style.background=thread.unread?"#f8f9ff":"white";}}>
                          <input type="checkbox" checked={selectedThreads.has(thread.id)}
                            onChange={()=>{const s=new Set(selectedThreads);s.has(thread.id)?s.delete(thread.id):s.add(thread.id);setSelectedThreads(s);setShowBulkActions(s.size>0);}}
                            onClick={(e)=>e.stopPropagation()}
                            className="h-4 w-4 rounded border-gray-300 mr-3 flex-shrink-0" style={{accentColor:"#1a73e8"}}/>
                          ...
                          <div className="flex-1 min-w-0 cursor-pointer py-1 select-none" onClick={()=>loadThread(thread.id)}>
                            <div className="flex items-center gap-2">
                              <span className="text-sm truncate w-40 flex-shrink-0" style={{color:thread.unread?"#202124":"#5f6368",fontWeight:thread.unread?600:400}}>
                                {extractName(thread.from)}
                              </span>
                              {/* <span className="text-sm truncate" style={{color:thread.unread?"#202124":"#5f6368"}}>
                                {thread.subject||(No Subject)"}
    ...
    <span className="font-normal ml-1.5" style={{color:"#80868b"}}>{thread.snippet}</span>
                          ...
                          <div className="flex items-center gap-2 ml-3 flex-shrink-0">
                            {isThisOpening?<FaSpinner className="animate-spin" size={12} style={{color:"#1a73e8"}}/>:thread.unread&&<div className="w-2 h-2 rounded-full" style={{background:"#1a73e8"}}/>}
                            <span className="text-xs whitespace-nowrap" style={{color:"#5f6368"}}>{formatDateTime(thread.date)}</span>
                            <button onClick={(e)=>{e.stopPropagation();setThreadToDelete({id:thread.id,permanent:activeLabel==="TRASH"});setShowDeleteModal(true);}}
                              className="opacity-0 group-hover:opacity-100 transition p-1 rounded-full hover:bg-gray-100" style={{color:"#5f6368"}}>
                              <FaTrash size={13}/>
                            </button>
                          </div>
                ...
                {nextPageTokens[activeLabel]&&filteredThreads.length>0&&(
                  <div className="mt-3 text-center">
                    <button onClick={()=>fetchLabel({label:activeLabel,loadMore:true})} disabled={listLoading&&listLoadLabel===activeLabel}
                      className="text-sm px-5 py-2 rounded-full border transition" style={{borderColor:"#dadce0",color:"#1a73e8",background:"white"}}>
                      {listLoading&&listLoadLabel===activeLabel?<><FaSpinner className="animate-spin inline mr-2" size={13}/>Loading...</>:<><FaPlus className="inline mr-2" size={13}/>Load More</>}
                    </button>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white rounded-2xl overflow-hidden" style={{border:"1px solid #e8eaed"}}>
                {threadLoading ? <MessageSkeleton/> : messages.length===0 ? (
                  <div className="text-center py-12">
                    <FaEnvelope size={46} className="mx-auto mb-3" style={{color:"#dadce0"}}/>
                    <p className="text-sm" style={{color:"#5f6368"}}>No messages</p>
                  </div>
                    ...
                    {messages.map((msg) => (
                      <div key={msg.id} className="p-6" style={{borderBottom:"1px solid #f1f3f4"}}>
                        <div className="flex items-start justify-between mb-4">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-xs" style={{color:"#5f6368"}}>{new Date(msg.date).toLocaleString()}</span>
                            <button onClick={()=>markThreadAs(selectedThread,"star",!msg.starred)} className="p-1.5 rounded-full hover:bg-gray-100" style={{color:msg.starred?"#f4b400":"#dadce0"}}><FaStar size={15}/></button>
                            <button onClick={()=>markThreadAs(selectedThread,"important",!msg.important)} className="p-1.5 rounded-full hover:bg-gray-100" style={{color:msg.important?"#f29900":"#dadce0"}}><FaExclamationCircle size={15}/></button>
                          </div>
                        </div>
                        {(msg.to||msg.cc)&&(
                          <div className="mb-4 text-xs" style={{color:"#5f6368"}}>
                              ...
                              {msg.attachments.map((att,idx) => (
                                <div key={idx} className="flex items-center justify-between p-2 bg-white rounded-lg" style={{border:"1px solid #e8eaed"}}>
                                  <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                      <p className="text-xs font-medium" style={{color:"#202124"}}>{att.filename}</p>
                                      <p className="text-xs" style={{color:"#5f6368"}}>{formatSize(att.size)}</p>
                                    </div>
                                  </div>
                                  <button onClick={()=>downloadAttachment(msg.id,att)} className="text-xs px-2.5 py-1 rounded-full flex items-center gap-1" style={{color:"#1a73e8",background:"#e8f0fe"}}>
                                    <FaDownload size={11}/>Download
                                  </button>
                                </div>
                              ))}
                        ...
                        <div className="mt-5 flex items-center gap-2 pt-4" style={{borderTop:"1px solid #f1f3f4"}}>
                          <button onClick={()=>openReply(msg,"reply")} className="text-sm px-4 py-1.5 rounded-full flex items-center gap-1.5" style={{background:"#c2e7ff",color:"#001d35"}}>
                            <FaReply size={13}/>Reply
                          </button>
                          <button onClick={()=>openReply(msg,"replyAll")} className="text-sm px-4 py-1.5 rounded-full border flex items-center gap-1.5 hover:bg-gray-50" style={{border:"1px solid #dadce0",color:"#444746"}}>
                            <FaReplyAll size={13}/>Reply All
                          </button>
                          <button onClick={()=>openReply(msg,"forward")} className="text-sm px-4 py-1.5 rounded-full border flex items-center gap-1.5 hover:bg-gray-50" style={{border:"1px solid #dadce0",color:"#444746"}}>
                            <FaForward size={13}/>Forward
                          </button>
                        </div>
                      </div>
      ...
      {showMobileSidebar&&<div className="fixed inset-0 bg-black bg-opacity-40 z-30 lg:hidden" onClick={()=>setShowMobileSidebar(false)}/>}
    </div>
  );
};
