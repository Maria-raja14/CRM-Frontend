...
// ── Animated background bubbles (unchanged) ───────────────────────────────────
const CardBubbles = ({ seed = 0, count = 12, colorPalette = ["#F59E0B", "#FBBF24", "#FCD34D"] }) => {
  const arr = Array.from({ length: count });
  return (
    <div className="absolute inset-0 pointer-events-none -z-0 overflow-hidden">
...
const StreakLeaderboard = () => {
  ...
  // ── Build start/end from selected month+year ─────────────────────────────────
  ...
  // ── SINGLE API CALL — reuses same leaderboard endpoint ──────────────────────
  const fetchTopPerformer = useCallback(async () => {
    ...
    try {
      ...
      // Backend already sorted by convertedLeads desc — first item = top performer
      const top = (data.data || [])[0] || null;
      setTopPerformer(top);
    } catch (err) {
      console.error("fetchTopPerformer error:", err);
      setError(err.response?.data?.error || err.message || "Failed to load top performer");
    } finally {
      setLoading(false);
    }
  ...
  // ── Render (UI identical to original) ────────────────────────────────────────
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
            ...
            >
              <div className="flex items-center gap-2">
                <select
                  value={selectedMonth}
                  onChange={e => setSelectedMonth(parseInt(e.target.value))}
                  className="text-xs bg-white border border-gray-200 rounded-lg px-2 py-1.5 focus:ring-1 focus:ring-orange-500 flex-1"
                >
                  {months.map((m, i) => (
            ...
            <div className="relative">
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-lg p-2 border border-orange-100">
                  <p className="text-xs text-gray-600 mb-1">Conversion</p>
                  <p className="text-lg font-bold text-orange-600">
                    {topPerformer.conversionDisplay}
                  </p>
                  <p className="text-xs text-gray-500">
                    {topPerformer.convertedLeads}/{topPerformer.totalLeads}
                  </p>
                </div>
                <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-2 border border-blue-100">
                  <p className="text-xs text-gray-600 mb-1">Active Days</p>
                  <p className="text-lg font-bold text-blue-600">
                    {topPerformer.productiveDays}
                  </p>
                  <p className="text-xs text-gray-500 truncate">
                    {topPerformer.workHours !== "—"
