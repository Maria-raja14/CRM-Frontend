...
// import { getSocket } from "../../utils/socket"; // adjust path to your socket util
...
//   <span className={`px-2 py-0.5 rounded-full text-xs font-semibold border capitalize ${STATUS_STYLES[status] || "bg-gray-100 text-gray-600 border-gray-200"}`}>
//     {status || "unknown"}
//   </span>
// );
...
//         <p className="text-gray-600 text-xs">{callData.callerName || cleanNumber(callData.from)}</p>
//         <p className="text-gray-400 text-xs">{cleanNumber(callData.from)}</p>
//       </div>
//     </div>
//     <button onClick={onDismiss} className="text-xs text-gray-400 hover:text-gray-600 underline self-end">
//       Dismiss
//     </button>
//   </div>
// );
...
//   // Notes modal
//   const [notesModal,  setNotesModal]  = useState(null); // { call }
...
//     try {
//       setLoading(true);
//       const data = await fetchCalls({ page, status: statusF, direction: directionF, search });
//       setCalls(data.calls || []);
...
//     searchTimer.current = setTimeout(() => { setPage(1); loadCalls(); }, 500);
...
//           closeOnClick: false,
//           icon: false,
//           style: { border: "2px solid #22c55e", borderRadius: "12px" },
//         }
//       );
//       // Add to top of list
//       setCalls((prev) => [
...
//             ? { ...c, status: data.status, duration: data.duration || c.duration }
//             : c
//         )
//       );
//     };
...
//       setCalls((prev) =>
//         prev.map((c) => c._id === notesModal.call._id ? { ...c, notes: notesText } : c)
//       );
...
//           <button
//             onClick={() => { setSearch(""); setStatusF(""); setDirectionF(""); }}
...
//         <span className="ml-auto text-sm text-gray-400">{total} call(s) found</span>
//       </div>
...
//                     <p className="text-xs">Configure the Twilio webhook to start receiving calls.</p>
//                   </div>
//                 </td>
//               </tr>
//             ) : (
...
//                   <td className="px-4 py-3">
//                     <div>
//                       <p className="font-medium text-gray-800">{cleanNumber(call.from)}</p>
//                       {call.callerName && (
...
//                   {/* To */}
//                   <td className="px-4 py-3 text-gray-700">{cleanNumber(call.to)}</td>
...
//                   <td className="px-4 py-3">
//                     <button
//                       onClick={() => { setNotesModal({ call }); setNotesText(call.notes || ""); }}
...
//                     page === p ? "bg-blue-600 text-white border-blue-600" : "hover:bg-gray-100 text-gray-700"
//                   }`}
//                 >
//                   {p}
//                 </button>
...
//       {/* ── Dial Modal ────────────────────────────────────────────────────── */}
//       {dialOpen && (
//         <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setDialOpen(false)}>
...
//                   autoFocus
//                 />
//                 <p className="text-xs text-gray-400 mt-1">Include country code or enter 10-digit Indian number</p>
//               </div>
//             </div>
//             <div className="px-5 pb-5 flex gap-3">
//               <button onClick={() => setDialOpen(false)} className="flex-1 py-2.5 border rounded-lg text-gray-600 text-sm hover:bg-gray-50">Cancel</button>
//               <button
...
//               <button onClick={() => setNotesModal(null)} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button>
//             </div>
//             <div className="p-5">
//               <p className="text-xs text-gray-400 mb-2">{cleanNumber(notesModal.call.from)} → {cleanNumber(notesModal.call.to)}</p>
//               <textarea
//                 value={notesText}
...
//             </div>
//             <div className="px-5 pb-5 flex gap-3">
//               <button onClick={() => setNotesModal(null)} className="flex-1 py-2 border rounded-lg text-gray-600 text-sm hover:bg-gray-50">Cancel</button>
//               <button onClick={handleSaveNotes} disabled={savingNotes} className="flex-1 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-60">
...
//               <br /><span className="text-xs text-gray-400">{formatDateTime(deleteTarget.createdAt)}</span>
//             </p>
//             <div className="flex gap-3">
//               <button onClick={() => setDeleteTarget(null)} className="flex-1 py-2.5 border rounded-lg text-gray-600 text-sm hover:bg-gray-50">Cancel</button>
//               <button onClick={handleDelete} className="flex-1 py-2.5 bg-red-600 text-white rounded-lg text-sm font-medium hover:bg-red-700">Delete</button>
//             </div>
//           </div>
//         </div>
//       )}
