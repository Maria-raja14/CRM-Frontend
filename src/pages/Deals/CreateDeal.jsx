...
//   { code: "INR", symbol: "₹",   label: "🇮🇳 INR" }, { code: "GBP", symbol: "£",   label: "🇬🇧 GBP" },
//   { code: "JPY", symbol: "¥",   label: "🇯🇵 JPY" }, { code: "AUD", symbol: "A$",  label: "🇦🇺 AUD" },
//   { code: "CAD", symbol: "C$",  label: "🇨🇦 CAD" }, { code: "CHF", symbol: "CHF", label: "🇨🇭 CHF" },
//   { code: "MYR", symbol: "RM",  label: "🇲🇾 MYR" }, { code: "AED", symbol: "د.إ", label: "🇦🇪 AED" },
//   { code: "SGD", symbol: "S$",  label: "🇸🇬 SGD" }, { code: "ZAR", symbol: "R",   label: "🇿🇦 ZAR" },
//   { code: "SAR", symbol: "﷼",   label: "🇸🇦 SAR" },
// ];
...
//   return "other";
// };
// const canPreview   = (name, mimeType) => ["image","pdf","text"].includes(getFileCategory(name, mimeType));
// const formatFileSize = (bytes) => {
//   if (!bytes) return "";
//   if (bytes < 1024)      return `${bytes} B`;
//   if (bytes < 1024*1024) return `${(bytes/1024).toFixed(1)} KB`;
...
//   const [content, setContent] = useState("Loading…");
//   useEffect(() => {
//     fetch(url).then((r) => r.text()).then(setContent).catch(() => setContent("Could not load file contents."));
//   }, [url]);
//   return (
//     <pre className="whitespace-pre-wrap text-sm text-slate-700 bg-white p-4 rounded-lg border border-slate-200 max-h-[60vh] overflow-auto font-mono">
//       {content}
...
//   return (
//     <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4" onClick={onClose}>
//       <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col overflow-hidden" style={{ maxHeight: "92vh" }} onClick={(e) => e.stopPropagation()}>
//         <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 flex-shrink-0">
//           <div className="flex items-center gap-3 min-w-0">
//             <FileText size={20} className="text-slate-500 flex-shrink-0" />
//             <span className="font-medium text-slate-900 truncate text-sm">{file.name}</span>
...
//   const [formData, setFormData] = useState({
//     dealName:    "", dealValue: "", currency: "INR",
//     stage:       "Qualification", assignTo: "", notes: "",
...
//   const totalPurchasing = parseCost(formData.purchasingLandCost) + parseCost(formData.purchasingTicketCost);
//   const totalSelling    = parseCost(formData.sellingLandCost)    + parseCost(formData.sellingTicketCost);
...
//       const existingSource   = existingDeal.source || "";
//       const isCustomSource   = existingSource && !SOURCE_OPTIONS.slice(0, -1).includes(existingSource);
//       if (isCustomSource) setCustomSource(existingSource);
...
//         sellingTicketCost:    existingDeal.sellingTicketCost    || "",
//       });
//       if (existingDeal.attachments?.length > 0) setExistingAttachments(existingDeal.attachments);
//     }
//   }, [isEditMode, existingDeal]);
...
//     const errorFields = Object.entries(newErrors).filter(([, v]) => v);
//     if (errorFields.length > 0) {
//       const refs = { dealName: dealNameRef, dealValue: dealValueRef, phoneNumber: phoneNumberRef, email: emailRef, destination: destinationRef };
//       if (refs[errorFields[0][0]]) scrollToElement(refs[errorFields[0][0]]);
//       if (newErrors.source) toast.error("Please enter a custom source");
...
//       const token       = localStorage.getItem("token");
//       const data        = new FormData();
//       const finalSource = formData.source === "Other" ? customSource.trim() : formData.source;

//       Object.keys(formData).forEach((key) => {
//         if (key === "attachments") return;
//         if (key === "source") data.append("source", finalSource);
...
//       if (isEditMode && existingDeal) {
//         await axios.patch(
//           `${API_URL}/deals/update-deal/${existingDeal._id}`, data,
//           { headers: { "Content-Type": "multipart/form-data", Authorization: `Bearer ${token}` } }
//         );
//         toast.success("Deal updated successfully");
//       } else {
//         await axios.post(
//           `${API_URL}/deals/createManual`, data,
//           { headers: { "Content-Type": "multipart/form-data", Authorization: `Bearer ${token}` } }
//         );
//         toast.success("Deal created successfully");
//       }
...
//   const formFields = [
//     { name: "stage", label: "Stage", icon: <Briefcase size={16} />, type: "select",
//       options: ["Qualification","Proposal","Negotiation","Closed Won","Closed Lost"] },
//     { name: "destination", label: "Destination", icon: <Building2 size={16} />, required: true },
//     { name: "duration",    label: "Duration",    icon: <BriefcaseBusiness size={16} />, type: "text", placeholder: "e.g., 3 months, 1 year" },
//     { name: "country",     label: "Country",     icon: <Globe size={16} />, type: "select", options: countries },
//   ];

//   /* ── A small reusable cost input ── */
//   const CostInput = ({ label, name, value, colorClass }) => (
//     <div>
//       <label className={`block text-xs font-medium mb-1 ${colorClass}`}>{label}</label>
//       <div className="relative">
//         <span className={`absolute left-3 top-1/2 -translate-y-1/2 text-xs font-medium ${colorClass}`}>
...
//             <ArrowLeft size={20} />
//           </button>
//           <h1 className="text-2xl font-bold text-gray-800">{isEditMode ? "Edit Deal" : "Create New Deal"}</h1>
//         </div>
...
//                   <FileText size={16} /> Deal Name <span className="text-red-500">*</span>
//                 </label>
//                 <input type="text" name="dealName" value={formData.dealName} onChange={handleChange} placeholder="Enter Deal Name"
//                   className={`w-full border rounded-lg px-3 py-2 text-sm bg-white focus:ring-2 focus:ring-blue-500 outline-none transition h-11 ${errors.dealName ? "border-red-500" : "border-gray-300"}`} />
//                 {errors.dealName && <p className="text-red-500 text-sm mt-1">Deal Name is required</p>}
//               </div>
...
//                 </label>
//                 <div className={`border rounded-lg ${errors.phoneNumber || phoneError ? "border-red-500" : "border-gray-300"}`}>
//                   <PhoneInput country="in" value={formData.phoneNumber?.replace("+", "")} onChange={handlePhoneChange}
...
//                 {formData.source === "Other" && (
//                   <div className="mt-2">
//                     <input type="text" placeholder="Enter custom source" value={customSource}
//                       onChange={(e) => { setCustomSource(e.target.value); if (e.target.value.trim()) setErrors((prev) => ({ ...prev, source: false })); }}
//                       className={`w-full border rounded-lg px-3 py-2 text-sm bg-white focus:ring-2 focus:ring-blue-500 outline-none transition h-11 ${errors.source ? "border-red-500" : "border-gray-300"}`} />
//                     {errors.source && <p className="text-red-500 text-sm mt-1">Please enter a custom source</p>}
//                   </div>
//                 )}
//               </div>
...
//               {/* Dynamic fields */}
//               {formFields.map((field) => (
//                 <div key={field.name} ref={field.name === "destination" ? destinationRef : null}>
//                   <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center gap-2">
//                     {field.icon} {field.label}
//                     {field.required && <span className="text-red-500">*</span>}
//                   </label>
...
//                   <FileText size={16} /> Requirement
//                 </label>
//                 <input type="text" name="requirement" value={formData.requirement || ""} onChange={handleChange} placeholder="Enter Requirement"
//                   className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:ring-2 focus:ring-blue-500 outline-none transition h-11" />
//               </div>
...
//                   <MapPin size={16} /> Address
//                 </label>
//                 <textarea name="address" rows={3} value={formData.address} onChange={handleChange} placeholder="Enter full address..."
//                   className="w-full px-3 py-2 rounded-lg border border-gray-300 bg-white shadow-sm text-sm placeholder-gray-400 transition resize-none" />
//               </div>
//             </div>
//           </div>
...
//           {/* ══════════════════════════════════════════════════════════
//               COST SECTION — Purchasing + Selling side by side
//               ══════════════════════════════════════════════════════════ */}
//           <div className="p-6 border border-gray-200 rounded-xl shadow-sm space-y-4">
//             <h2 className="text-lg font-semibold border-b pb-2 text-purple-600">Cost Details</h2>
...
//                   <CostInput label="Land Part" name="purchasingLandCost"   value={formData.purchasingLandCost}   colorClass="text-orange-600" />
//                   <CostInput label="Ticket"    name="purchasingTicketCost" value={formData.purchasingTicketCost} colorClass="text-orange-600" />
//                 </div>
//                 <div className="mt-4 pt-3 border-t border-orange-200 flex justify-between items-center">
//                   <span className="text-sm text-orange-600 font-medium">Total Purchasing</span>
//                   <span className="text-base font-bold text-orange-700">
//                     {currencyOptions.find((c) => c.code === formData.currency)?.symbol || "₹"}
//                     {totalPurchasing.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
//                   </span>
//                 </div>
//               </div>
...
//                 </h3>
//                 <div className="grid grid-cols-2 gap-4">
//                   <CostInput label="Land Part" name="sellingLandCost"   value={formData.sellingLandCost}   colorClass="text-green-600" />
//                   <CostInput label="Ticket"    name="sellingTicketCost" value={formData.sellingTicketCost} colorClass="text-green-600" />
//                 </div>
//                 <div className="mt-4 pt-3 border-t border-green-200 flex justify-between items-center">
//                   <span className="text-sm text-green-600 font-medium">Total Selling</span>
//                   <span className="text-base font-bold text-green-700">
//                     {currencyOptions.find((c) => c.code === formData.currency)?.symbol || "₹"}
//                     {totalSelling.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
//                   </span>
//                 </div>
//               </div>
//             </div>
...
//                   {profit >= 0 ? <TrendingUp size={18} className="text-emerald-600" /> : <TrendingDown size={18} className="text-red-600" />}
//                   <span className={`text-sm font-semibold ${profit >= 0 ? "text-emerald-700" : "text-red-700"}`}>
//                     {profit >= 0 ? "Net Profit" : "Net Loss"}
//                   </span>
//                 </div>
//                 <span className={`text-lg font-bold ${profit >= 0 ? "text-emerald-700" : "text-red-700"}`}>
//                   {profit >= 0 ? "+" : "-"}
//                   {currencyOptions.find((c) => c.code === formData.currency)?.symbol || "₹"}
...
//               <StickyNote size={16} /> Notes
//             </label>
//             <textarea name="notes" rows={4} value={formData.notes} onChange={handleChange} placeholder="Enter Notes..."
//               className="w-full px-3 py-2 rounded-lg border border-gray-300 bg-white shadow-sm text-sm placeholder-gray-400 transition resize-none" />
//           </div>
...
//                     const showPreviewBtn = canPreview(fileName, mimeType);
//                     return (
//                       <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 border border-gray-200 rounded-lg hover:bg-gray-100 transition">
//                         <div className="flex items-center gap-3 min-w-0 flex-1">
//                           <div className={`w-8 h-8 flex items-center justify-center rounded-md flex-shrink-0 ${style.bg}`}>
//                             <FileText size={16} className={style.text} />
//                           </div>
//                           <div className="min-w-0">
//                             <p className="text-sm font-medium text-gray-700 truncate">{fileName}</p>
//                             <p className="text-xs text-gray-500">{cat.toUpperCase()}{fileSize > 0 && ` • ${formatFileSize(fileSize)}`}</p>
//                           </div>
//                         </div>
//                         <div className="flex items-center gap-1 ml-3 flex-shrink-0">
//                           {showPreviewBtn && (
//                             <button type="button" onClick={() => handleFilePreview(filePath, fileName, mimeType, `ex-${idx}`)}
//                               disabled={previewLoading === `ex-${idx}`}
//                               className="flex items-center gap-1 px-2 py-1.5 text-xs text-slate-600 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors">
//                               {previewLoading === `ex-${idx}` ? <span className="w-4 h-4 border-2 border-emerald-400 border-t-transparent rounded-full animate-spin inline-block" /> : <Eye size={14} />}
//                               <span className="hidden sm:inline">Preview</span>
//                             </button>
//                           )}
//                           <button type="button" onClick={() => handleFileDownload(filePath, fileName)}
//                             className="flex items-center gap-1 px-2 py-1.5 text-xs text-slate-600 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
...
//                         </div>
//                       </div>
//                       <button type="button" onClick={() => handleRemoveFile(idx, "new")} className="p-1.5 text-red-500 hover:text-red-700 ml-3 rounded-lg hover:bg-red-50 transition-colors">
//                         <X size={14} />
//                       </button>
//                     </div>
//                   ))}
...
//     destination: "", duration: "", requirement: "",
//     address: "", country: "", attachments: [],
//     /* ── Cost fields — accept text OR numbers ── */
//     purchasingLandCost:   "",
//     purchasingTicketCost: "",
//     sellingLandCost:      "",
//     sellingTicketCost:    "",
...
//   /* ── Numeric-safe cost parser (allows text values) ── */
//   const parseCost      = (v) => { const n = parseFloat(String(v || "").replace(/,/g, "")); return isNaN(n) ? 0 : n; };
//   const totalPurchasing = parseCost(formData.purchasingLandCost) + parseCost(formData.purchasingTicketCost);
//   const totalSelling    = parseCost(formData.sellingLandCost)    + parseCost(formData.sellingTicketCost);
//   const profit          = totalSelling - totalPurchasing;
//   const hasCostInput    = totalPurchasing > 0 || totalSelling > 0 ||
//                           formData.purchasingLandCost || formData.purchasingTicketCost ||
//                           formData.sellingLandCost    || formData.sellingTicketCost;
...
//   /* ── Reusable cost input (text + number) ── */
//   const CostInput = ({ label, name, value }) => (
//     <div>
...
//   const currSymbol = currencyOptions.find((c) => c.code === formData.currency)?.symbol || "₹";
...
//           {/* ══════════════════════════════════════════════════════════
//               COST SECTION — No coloring, Land Part supports text+numbers
//               ══════════════════════════════════════════════════════════ */}
...
//                 </div>
//                 <div className="grid grid-cols-2 gap-4">
//                   <CostInput label="Land Part" name="purchasingLandCost"   value={formData.purchasingLandCost} />
//                   <CostInput label="Ticket"    name="purchasingTicketCost" value={formData.purchasingTicketCost} />
//                 </div>
//                 <div className="mt-4 flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5">
//                   <span className="text-xs font-medium text-gray-500">Total Purchasing</span>
//                   <span className="text-sm font-bold text-gray-700">
//                     {currSymbol}{totalPurchasing.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
//                   </span>
//                 </div>
//               </div>
...
//                 </div>
//                 <div className="grid grid-cols-2 gap-4">
//                   <CostInput label="Land Part" name="sellingLandCost"   value={formData.sellingLandCost} />
//                   <CostInput label="Ticket"    name="sellingTicketCost" value={formData.sellingTicketCost} />
//                 </div>
//                 <div className="mt-4 flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5">
//                   <span className="text-xs font-medium text-gray-500">Total Selling</span>
//                   <span className="text-sm font-bold text-gray-700">
//                     {currSymbol}{totalSelling.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
//                   </span>
//                 </div>
//               </div>
//             </div>
...
//                 </div>
//                 <span className={`text-lg font-bold ${profit >= 0 ? "text-emerald-700" : "text-red-700"}`}>
//                   {profit >= 0 ? "+" : "-"}{currSymbol}{Math.abs(profit).toLocaleString("en-IN", { minimumFractionDigits: 2 })}
//                 </span>
//               </div>
//             )}
//           </div>
...
// /* ── Memoized Cost Input Component ── */
// const CostInput = memo(({ label, name, value, onChange }) => (
//   <div>
//     <label className="block text-xs font-medium text-gray-600 mb-1.5">{label}</label>
//     <input
//       type="text"
...
//                 </div>
//                 <div className="grid grid-cols-2 gap-4">
//                   <CostInput label="Land Part" name="purchasingLandCost" value={formData.purchasingLandCost} onChange={handleChange} />
//                   <CostInput label="Ticket"    name="purchasingTicketCost" value={formData.purchasingTicketCost} onChange={handleChange} />
//                 </div>
//                 <div className="mt-4 flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5">
//                   <span className="text-xs font-medium text-gray-500">Total Purchasing</span>
//                   <span className="text-sm font-bold text-gray-700">
...
//                 </div>
//                 <div className="grid grid-cols-2 gap-4">
//                   <CostInput label="Land Part" name="sellingLandCost"   value={formData.sellingLandCost} onChange={handleChange} />
//                   <CostInput label="Ticket"    name="sellingTicketCost" value={formData.sellingTicketCost} onChange={handleChange} />
//                 </div>
//                 <div className="mt-4 flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5">
//                   <span className="text-xs font-medium text-gray-500">Total Selling</span>
//                   <span className="text-sm font-bold text-gray-700">
    ...
    const fetchAssignableUsers = async () => {
      try {
        ...
        const [salesRes, opsRes] = await Promise.all([
          ...
          axios.get(`${API_URL}/users/operations`, { headers: { Authorization: `Bearer ${token}` } })
        ]);
        const combined = [...(salesRes.data.users || []), ...(opsRes.data.users || [])];
