...
// // ✅ FIXED: Centralized image URL builder — prevents double-path issues
...
//       // ✅ FIXED: Use centralized builder — add cache-busting timestamp
...
export default function EditUserModal({ user, roles, onClose, onUserUpdated }) {
  ...
  const validateForm = () => {
    ...
    // ✅ Email is now optional — only validate format if something is typed
  ...
  return (
    <Dialog open={isDialogOpen} onOpenChange={handleOpenChange}>
      <DialogContent
        ...
        className="sm:max-w-3xl max-h-[90vh] overflow-y-auto p-0"
        style={{ zIndex: 50 }}
        onPointerDownOutside={(e) => { if (isToastClick(e)) e.preventDefault(); }}
        onInteractOutside={(e)    => { if (isToastClick(e)) e.preventDefault(); }}
      >
        <DialogHeader className="px-6 pt-6 pb-0">
          <div className="flex justify-between items-center">
            <DialogTitle className="text-xl font-bold text-gray-800">
        ...
        >
          ...
          {/* ✅ Email — optional, no asterisk, no mandatory validation */}
          <div>
            <label className="text-sm font-medium text-gray-700 mb-1 block">
              Email
            </label>
            ...
            />
            {/* Only shows if format is wrong, not if empty */}
            {errors.email && (
              <p className="text-red-500 text-sm mt-1.5 flex items-center gap-1">
