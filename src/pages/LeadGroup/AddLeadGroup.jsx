...
const AddLeadGroup = ({ onClose, onAdd,leadGroupToEdit  }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-opacity-40  bg-opacity-50 backdrop-blur-sm z-50 ">
      <div className="bg-white w-[600px] h-[400px] rounded-lg shadow-lg flex flex-col">
        <div className="flex justify-between items-center pb-3 p-8">
          <h2 className="text-xl font-semibold shadow-2xl ">Add Lead Group</h2>
          <button className="text-black font-bold hover:text-gray-600 cursor-pointer" onClick={onClose}>
            X
          </button>
        </div>
        ...
        {/* Footer Buttons */}
        <div className="flex justify-end gap-2 mt-12 mr-2 p-3">
          <button className="bg-gray-300 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-400 cursor-pointer" onClick={onClose}>
            Cancel
          </button>
          <button
            onClick={handleSubmit}
            disabled={loading}
            className={`px-4 py-2 rounded-md text-white shadow-lg cursor-pointer ${loading ? "bg-gray-500" : "bg-blue-600 hover:bg-blue-700"}`}
          >
            {loading ? "Saving..." : "Save"}
          </button>
        </div>
