...
//   const url = `${API_URL.replace(/\/$/, "")}${filePath}`;
...
//                                 {lead.assignTo.lastName} ({lead.assignTo.email})
//                               </p>
//                             </div>
//                           </div>
//                         )}
...
const downloadFile = (filePath, fileName) => {
  ...
  const url = `${API_SI.replace(/\/$/, "")}${filePath}`;
  ...
  onClick={() => downloadFile(file.path)}
  className={`p-2 text-slate-600 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors ${
    !file.path ? "opacity-50 cursor-not-allowed" : ""
  }`}
  title={file.path ? "Download" : "File path missing"}
>
  <Download size={18} />
</button>
