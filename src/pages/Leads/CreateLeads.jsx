...
export default function CreateLeads() {
  ...
  useEffect(() => {
    if (leadId) {
      const fetchLead = async () => {
        try {
          const response = await axios.get(`http://localhost:5000/api/leads/getLead/${leadId}`);
  ...
  const fetchSalesUsers = async () => {
    try {
      ...
      const filteredSales = response.data.filter(
        (user) => user.role && user.role.name && user.role.name.toLowerCase() === "sales"
      );

      console.log("Filtered Sales Users:", filteredSales);
  ...
  const fieldGroups = [
    { title: "Basic Information", fields: ["leadName", "companyName", "phoneNumber", "email", "address"] },
    { title: "Business Details", fields: ["industry", "source", "requirement"] },
    { title: "Lead Management", fields: ["status", "assignTo", "priorityLevel", "followUpDate", "leadStatus"] },
                      ...
                      <div key={field} className={`space-y-2 ${field === 'notes' ? 'md:col-span-2 lg:col-span-3' : ''}`}>
                        ...
                        {getFieldOptions(field) ? (
                          <select
                            ...
                            onChange={handleChange}
                            className={`block w-full px-4 py-3 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm ${
                              errors[field] ? "border-red-300 text-red-900 placeholder-red-300 focus:ring-red-500 focus:border-red-500" : "border-gray-300"
                            }`}
                          >
                            <option value="">Select {field.replace(/([A-Z])/g, ' $1').trim()}</option>
                            {getFieldOptions(field).map((option) =>
