...
//   { selector: ".tour-create-lead",  content: "Click here to create a new lead. You'll be able to add all the necessary details about a potential customer." },
//   { selector: ".tour-search",       content: "Use this search bar to quickly find leads by name, email, phone, or source." },
//   { selector: ".tour-filters",      content: "Filter your leads by status, assignee, or source to focus on specific segments of your pipeline." },
//   { selector: ".tour-lead-table",   content: "This is your leads table. It shows all your leads with their key information and status." },
//   { selector: ".tour-checkbox",     content: "Select individual leads by checking these boxes, or use the header checkbox to select all visible leads." },
//   { selector: ".tour-lead-actions", content: "Click the three-dot menu to edit, convert, or delete a lead. Converting a lead turns it into a deal." },
...
//         lead.email?.toLowerCase().includes(q)       ||
//         lead.phoneNumber?.includes(searchQuery)     ||
//         lead.destination?.toLowerCase().includes(q) || // ✅ renamed from companyName
//         lead.source?.toLowerCase().includes(q);
...
//   const handleEdit        = (leadId) => { navigate(`/createleads?id=${leadId}`); setMenuOpen(null); };
//   const handleDeleteClick = (leadId) => { setLeadToDelete(leadId); setShowDeleteModal(true); setMenuOpen(null); };
...
//       if (allSuccess) {
//         setLeads((prev)    => prev.filter((l) => !selectedLeads.includes(l._id)));
//         setAllLeads((prev) => prev.filter((l) => !selectedLeads.includes(l._id)));
//         toast.success(`${selectedLeads.length} leads deleted successfully`);
//         setSelectedLeads([]);
//         if (leads.length === selectedLeads.length && currentPage > 1)
//           setCurrentPage(currentPage - 1);
...
//       if (res.status === 200) {
//         const patch = (list) =>
//           list.map((l) => (l._id === leadId ? { ...l, status: newStatus } : l));
//         setLeads((prev)    => patch(prev));
//         setAllLeads((prev) => patch(prev));
//         toast.success("Status updated successfully");
//       }
...
//   const getStatusSelectClass = (status) =>
//     `w-full px-3 py-1.5 rounded-full text-xs font-medium border transition-all duration-200 cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-1 ${
...
//       status === "Junk" ? "focus:ring-gray-300"   : "focus:ring-green-300"
//     }`;
...
//           {(userRole === "Admin" || userRole === "Sales") && (
//             <button className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium shadow flex items-center gap-2 tour-create-lead"
//               onClick={() => navigate("/createleads")}>
//               <Plus className="w-4 h-4" /> Create Lead
//             </button>
//           )}
...
//               {/* ✅ Column header renamed to Destination */}
...
//                   {/* ✅ KEY FIX: reads lead.destination instead of lead.companyName */}
...
//                         className="inline-flex items-center gap-2 px-2 py-1 rounded-md hover:bg-gray-100 transition"
//                         title="Click to update follow-up date" disabled={followUpSavingId === lead._id}>
//                         <Calendar className="w-4 h-4 text-gray-500" />
//                         <span className="text-sm">
...
//               to <span className="font-medium">{Math.min(currentPage * itemsPerPage, totalLeads)}</span>{" "}
//               of <span className="font-medium">{totalLeads}</span> results
//             </p>
//           </div>
//           <div className="flex gap-2">
...
//                 <p className="text-sm text-blue-800">
//                   Converting: <strong>{selectedLead.leadName}</strong>
//                   {/* ✅ shows destination instead of companyName */}
...
//                 <select name="stage" value={dealData.stage}
//                   onChange={(e) => handleDealFieldChange("stage", e.target.value)}
//                   className="w-full px-3 py-2 border rounded-md focus:ring-2 focus:ring-green-500 focus:outline-none">
//                   <option value="Qualification">Qualification</option>
//                   <option value="Proposal">Proposal</option>
//                   <option value="Negotiation">Negotiation</option>
...
//                 <textarea name="notes" value={dealData.notes}
