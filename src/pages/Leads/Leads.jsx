...
//     try {
//       const token = localStorage.getItem("token");
//       await axios.delete(`${API_URL}/leads/deleteLead/${id}`, { headers: { Authorization: `Bearer ${token}` } });
//       setLeads((prev) => prev.filter((l) => l._id !== id));
//       setTotalLeads((prev) => prev - 1);
//       toast.success("Lead deleted successfully");
//       if (leads.length === 1 && currentPage > 1) setCurrentPage((p) => p - 1);
//     } catch { toast.error("Error deleting lead"); }
//     finally { setShowDeleteModal(false); setLeadToDelete(null); }
//   };
...
//         axios.delete(`${API_URL}/leads/deleteLead/${id}`, { headers: { Authorization: `Bearer ${token}` } })
//       ));
//       setLeads((prev) => prev.filter((l) => !selectedLeads.includes(l._id)));
//       setTotalLeads((prev) => prev - selectedLeads.length);
//       toast.success(`${selectedLeads.length} leads deleted`);
...
//     } catch { toast.error("Error deleting leads"); }
//     finally { setShowDeleteModal(false); }
//   };

//   const handleSelectLead  = (id) => setSelectedLeads((prev) => prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]);
//   const handleSelectAll   = (e) => setSelectedLeads(e.target.checked ? leads.map((l) => l._id) : []);
...
//   /* ── Cost helpers ── */
//   const parseCost = (v) => parseFloat(String(v || "").replace(/,/g, "")) || 0;
...
//         </div>
//         {userRole === "Admin" && (
//           <select value={assigneeFilter} onChange={(e) => setAssigneeFilter(e.target.value)} className="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
//             <option value="">All Assignees</option>
//             {assignees.map((a, i) => <option key={i} value={a}>{a}</option>)}
//           </select>
//         )}
//         <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
//           <option value="">All Status</option>
//           <option value="Hot">Hot</option><option value="Warm">Warm</option>
//           <option value="Cold">Cold</option><option value="Junk">Junk</option>
//           <option value="Converted">Converted</option>
//         </select>
//         <select value={sourceFilter} onChange={(e) => setSourceFilter(e.target.value)} className="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
//           <option value="">All Sources</option>
//           <option value="Website">Website</option><option value="Referral">Referral</option>
//           <option value="Social Media">Social Media</option><option value="Email">Email</option>
...
//                   checked={leads.length > 0 && selectedLeads.length === leads.length} onChange={handleSelectAll} />
//               </th>
//               <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Lead</th>
//               <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Contact</th>
//               <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Destination</th>
...
//           <tbody className="divide-y divide-gray-200 bg-white">
//             {leads.length > 0 ? leads.map((lead, idx) => (
//               <tr key={lead._id} className={`hover:bg-blue-50/30 transition-colors whitespace-nowrap ${selectedLeads.includes(lead._id) ? "bg-blue-50" : idx % 2 === 0 ? "bg-white" : "bg-gray-50/50"}`}>
//                 <td className="px-4 py-3">
//                   <input type="checkbox" className="h-4 w-4 text-blue-600 border-gray-300 rounded"
//                     checked={selectedLeads.includes(lead._id)} onChange={() => handleSelectLead(lead._id)} />
//                 </td>
//                 <td className="px-4 py-3">
//                   <div className="flex items-center gap-3">
//                     <LeadAvatar name={lead.leadName} />
...
//                         className="font-medium text-blue-600 text-sm cursor-pointer hover:underline truncate max-w-[160px]" title={lead.leadName || "Unnamed Lead"}>
//                         {lead.leadName || "Unnamed Lead"}
//                       </span>
...
//                 <td className="px-4 py-3 text-sm text-gray-700">{lead.source       || "-"}</td>
//                 <td className="px-4 py-3">
//                   <select value={lead.status} onChange={(e) => handleStatusChange(lead._id, e.target.value)} className={getStatusClass(lead.status)}>
//                     <option value="Hot">Hot</option><option value="Warm">Warm</option>
//                     <option value="Cold">Cold</option><option value="Junk">Junk</option>
//                   </select>
//                 </td>
//                 <td className="px-4 py-3 text-sm text-gray-700">
//                   {lead.assignTo ? typeof lead.assignTo === "object" ? `${lead.assignTo.firstName} ${lead.assignTo.lastName}` : "Assigned User" : "-"}
//                 </td>
//                 <td className="px-4 py-3 text-sm text-gray-700">{formatDate(lead.createdAt)}</td>
//                 <td className="px-4 py-3 text-sm text-gray-700">
//                   <div className="relative flex items-center gap-1">
//                     <button type="button" onClick={() => openFollowUpPicker(lead._id)} disabled={followUpSavingId === lead._id}
//                       className="inline-flex items-center gap-2 px-2 py-1 rounded-md hover:bg-gray-100 transition">
//                       <Calendar className="w-4 h-4 text-gray-500" />
//                       <span className="text-sm">{followUpSavingId === lead._id ? "Saving..." : formatDate(lead.followUpDate)}</span>
//                     </button>
//                     {editingFollowUpId === lead._id && (
//                       <input ref={(el) => (dateInputRefs.current[lead._id] = el)} type="date"
//                         defaultValue={toDateInputValue(lead.followUpDate)}
//                         className="absolute left-0 top-0 w-0 h-0 opacity-0"
//                         onChange={(e) => updateFollowUpDateInline(lead._id, e.target.value)}
//                         onBlur={() => setEditingFollowUpId(null)} />
...
//                 </td>
//                 <td className="px-4 py-3 text-right relative">
//                   <button className="p-2 rounded-lg hover:bg-gray-200 transition-colors" onClick={(e) => handleMenuToggle(lead._id, e)}>
//                     <MoreVertical className="w-5 h-5 text-gray-600" />
//                   </button>
//                   {menuOpen === lead._id && (
//                     <div className="fixed z-50 w-40 bg-white rounded-lg shadow-lg border border-gray-200 py-1" style={{ top: `${menuPosition.top}px`, left: `${menuPosition.left}px` }}>
//                       <button onClick={(e) => { e.stopPropagation(); handleEdit(lead._id); }} className="flex items-center w-full px-3 py-2 text-sm text-gray-700 hover:bg-gray-100">
//                         <Edit className="w-4 h-4 mr-2" /> Edit
//                       </button>
//                       {lead.status !== "Converted" && (
//                         <button onClick={(e) => { e.stopPropagation(); openConvertModal(lead); }} className="flex items-center w-full px-3 py-2 text-sm text-green-600 hover:bg-gray-100">
//                           <Handshake className="w-4 h-4 mr-2" /> Convert
//                         </button>
//                       )}
//                       <button onClick={(e) => { e.stopPropagation(); handleDeleteClick(lead._id); }} className="flex items-center w-full px-3 py-2 text-sm text-red-600 hover:bg-gray-100">
//                         <Trash2 className="w-4 h-4 mr-2" /> Delete
//                       </button>
//                     </div>
//                   )}
...
//           </p>
//           <div className="flex justify-end gap-3">
//             <button onClick={() => { setShowDeleteModal(false); setLeadToDelete(null); }} className="px-4 py-2 rounded-lg border hover:bg-gray-100 text-gray-700">Cancel</button>
//             <button onClick={() => leadToDelete ? handleDeleteLead(leadToDelete) : handleBulkDelete()} className="px-4 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700 flex items-center gap-2">
//               <Trash2 className="w-4 h-4" /> Delete
//             </button>
//           </div>
//         </DialogContent>
...
//       {/* ── Convert Modal — HORIZONTAL WIDE LAYOUT ── */}
//       <Dialog open={convertModalOpen} onOpenChange={setConvertModalOpen}>
...
//                       onChange={(e) => setDealData((p) => ({ ...p, value: e.target.value }))}
//                       placeholder="Enter value"
//                       className="flex-1 px-3 py-2 border rounded-md focus:ring-2 focus:ring-green-500 focus:outline-none text-sm"
//                     />
//                     <select value={dealData.currency} onChange={(e) => setDealData((p) => ({ ...p, currency: e.target.value }))}
//                       className="px-2 py-2 border rounded-md focus:ring-2 focus:ring-green-500 focus:outline-none text-sm">
...
//                 <div>
//                   <label className="block text-sm font-medium text-gray-700 mb-1">Stage</label>
//                   <select value={dealData.stage} onChange={(e) => setDealData((p) => ({ ...p, stage: e.target.value }))}
...
//               {/* ── Row 2: Purchasing Cost | Selling Cost — side by side ── */}
//               <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">

//                 {/* Purchasing Cost card */}
...
//                     <div>
//                       <label className="block text-xs font-medium text-gray-600 mb-1">Land Part</label>
//                       <input
//                         type="number" min="0" placeholder="0.00"
//                         value={dealData.purchasingLandCost}
//                         onChange={(e) => setDealData((p) => ({ ...p, purchasingLandCost: e.target.value }))}
//                         className="w-full px-3 py-2 border border-orange-200 rounded-lg text-sm focus:ring-2 focus:ring-orange-400 focus:outline-none bg-white"
//                       />
//                     </div>
//                     <div>
...
//                       <input
//                         type="number" min="0" placeholder="0.00"
//                         value={dealData.purchasingTicketCost}
...
//                         <span>{dealData.currency} {totalPurchasing.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
//                       </div>
//                     </div>
//                   </div>
//                 </div>
...
//                       <input
//                         type="number" min="0" placeholder="0.00"
//                         value={dealData.sellingLandCost}
//                         onChange={(e) => setDealData((p) => ({ ...p, sellingLandCost: e.target.value }))}
//                         className="w-full px-3 py-2 border border-green-200 rounded-lg text-sm focus:ring-2 focus:ring-green-400 focus:outline-none bg-white"
//                       />
//                     </div>
//                     <div>
//                       <label className="block text-xs font-medium text-gray-600 mb-1">Ticket</label>
//                       <input
//                         type="number" min="0" placeholder="0.00"
//                         value={dealData.sellingTicketCost}
...
//                         <span>{dealData.currency} {totalSelling.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
//                       </div>
//                     </div>
//                   </div>
//                 </div>
...
//               <div className={`mb-4 px-4 py-3 rounded-lg flex items-center justify-between ${profit >= 0 ? "bg-emerald-50 border border-emerald-200" : "bg-red-50 border border-red-200"}`}>
//                 <span className={`text-sm font-semibold ${profit >= 0 ? "text-emerald-700" : "text-red-700"}`}>
//                   {profit >= 0 ? "💰 Profit" : "⚠️ Loss"}
//                 </span>
//                 <span className={`text-base font-bold ${profit >= 0 ? "text-emerald-700" : "text-red-700"}`}>
//                   {dealData.currency} {Math.abs(profit).toLocaleString("en-IN", { minimumFractionDigits: 2 })}
//                 </span>
//               </div>
...
//               <div className="mb-4">
//                 <label className="block text-sm font-medium text-gray-700 mb-1">Notes</label>
//                 <textarea value={dealData.notes} rows={2} placeholder="Add any notes..."
//                   onChange={(e) => setDealData((p) => ({ ...p, notes: e.target.value }))}
//                   className="w-full px-3 py-2 border rounded-md focus:ring-2 focus:ring-green-500 focus:outline-none text-sm" />
//               </div>
...
//       {/* ══════════════════════════════════════════════════════════
//           CONVERT MODAL — HORIZONTAL FLAT LAYOUT (no card colors)
//           ══════════════════════════════════════════════════════════ */}
//       <Dialog open={convertModalOpen} onOpenChange={setConvertModalOpen}>
...
//                     />
//                     <select value={dealData.currency} onChange={(e) => setDealData((p) => ({ ...p, currency: e.target.value }))}
//                       className="px-2 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none text-sm">
//                       {allowedCurrencies.map((c) => <option key={c.code} value={c.code}>{c.symbol} {c.code}</option>)}
//                     </select>
//                   </div>
//                 </div>
...
//               {/* ── Row 2: Cost fields — all 4 in one horizontal row ── */}
//               <div className="mb-2">
...
//                     <input
//                       type="text"
//                       placeholder="e.g. 7000 or Included"
//                       value={dealData.sellingLandCost}
//                       onChange={(e) => setDealData((p) => ({ ...p, sellingLandCost: e.target.value }))}
//                       className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
//                     />
...
//                 <div className="col-span-2 flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg px-4 py-2">
//                   <span className="text-xs font-medium text-gray-500">Total Purchasing</span>
//                   <span className="text-sm font-bold text-gray-700">
//                     {dealData.currency} {totalPurchasing.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
//                   </span>
//                 </div>
//                 <div className="col-span-2 flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg px-4 py-2">
//                   <span className="text-xs font-medium text-gray-500">Total Selling</span>
//                   <span className="text-sm font-bold text-gray-700">
//                     {dealData.currency} {totalSelling.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
//                   </span>
//                 </div>
//               </div>
...
//                 <div className={`mb-4 px-4 py-3 rounded-lg flex items-center justify-between border ${profit >= 0 ? "bg-emerald-50 border-emerald-200" : "bg-red-50 border-red-200"}`}>
//                   <span className={`text-sm font-semibold ${profit >= 0 ? "text-emerald-700" : "text-red-700"}`}>
//                     {profit >= 0 ? "💰 Profit" : "⚠️ Loss"}
//                   </span>
//                   <span className={`text-base font-bold ${profit >= 0 ? "text-emerald-700" : "text-red-700"}`}>
...
//     try {
//       const token = localStorage.getItem("token");
//       await axios.patch(`${API_URL}/leads/${leadId}/status`, { status: newStatus }, { headers: { Authorization: `Bearer ${token}` } });
//       setLeads((prev) => prev.map((l) => l._id === leadId ? { ...l, status: newStatus } : l));
//       toast.success("Status updated");
//     } catch { toast.error("Failed to update status"); }
//   };
...
//       setFollowUpSavingId(leadId);
//       const token = localStorage.getItem("token");
//       await axios.patch(`${API_URL}/leads/${leadId}/followup`, { followUpDate: newDate }, { headers: { Authorization: `Bearer ${token}` } });
//       setLeads((prev) => prev.map((l) => l._id === leadId ? { ...l, followUpDate: newDate } : l));
//       toast.success("Follow-up date updated");
//     } catch (err) {
//       toast.error(err.response?.data?.message || "Failed to update follow-up date");
...
//       {/* ══════════════════════════════════════════════════════════════
//           CONVERT MODAL — WIDE HORIZONTAL, NO CARD COLORS
//           Land Part fields accept both text and numbers
//           ══════════════════════════════════════════════════════════════ */}
//       <Dialog open={convertModalOpen} onOpenChange={setConvertModalOpen}>
...
//                       onChange={(e) => setDealData((p) => ({ ...p, value: e.target.value }))}
//                       placeholder="Enter value"
//                       className="flex-1 px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-400 focus:outline-none"
//                     />
//                   </div>
//                 </div>
//                 <div>
...
//                   <span className="text-xs font-bold text-gray-600 uppercase tracking-wide">Purchasing Cost</span>
//                   <div className="flex-1 h-px bg-gray-200" />
//                 </div>
//                 <div className="flex items-center gap-2">
//                   <span className="text-xs font-bold text-gray-600 uppercase tracking-wide">Selling Cost</span>
//                   <div className="flex-1 h-px bg-gray-200" />
//                 </div>
//               </div>

//               {/* ── All 4 cost fields in one horizontal row ── */}
//               <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
//                 {/* Purchasing Land */}
//                 <div>
...
//               {/* ── Net Profit / Loss ── */}
//               {(totalPurchasing > 0 || totalSelling > 0) && (
//                 <div className={`flex items-center justify-between px-4 py-3 rounded-lg border ${profit >= 0 ? "bg-emerald-50 border-emerald-200" : "bg-red-50 border-red-200"}`}>
//                   <div className="flex items-center gap-2">
...
//                   </div>
//                   <span className={`text-base font-bold ${profit >= 0 ? "text-emerald-700" : "text-red-700"}`}>
//                     {profit >= 0 ? "+" : "-"}{dealData.currency} {Math.abs(profit).toLocaleString("en-IN", { minimumFractionDigits: 2 })}
//                   </span>
//                 </div>
//               )}
...
//                 <textarea
//                   value={dealData.notes} rows={2} placeholder="Add any notes..."
//                   onChange={(e) => setDealData((p) => ({ ...p, notes: e.target.value }))}
//                   className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-400 focus:outline-none resize-none"
//                 />
//               </div>
...
//                 <button onClick={() => setConvertModalOpen(false)} disabled={converting}
//                   className="px-5 py-2 rounded-lg border border-gray-300 hover:bg-gray-100 text-gray-700 text-sm font-medium">Cancel</button>
//                 <button onClick={handleConvertDeal} disabled={converting}
...
import {
  MoreVertical, Trash2, Edit, Handshake, Search, Plus, Eye, Calendar,
  TrendingUp, TrendingDown, Facebook,
} from "lucide-react";
import { initSocket } from "../../utils/socket";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "../../components/ui/dialog";
...
const LeadAvatar = ({ name, isFacebook }) => {
  const initials = getInitials(name);
  const color    = getAvatarColor(name);
  return (
    <div className="relative flex-shrink-0">
      <div
        className={`h-9 w-9 rounded-full flex items-center justify-center font-semibold text-xs select-none ${color.bg} ${color.text}`}
        style={{ minWidth: "2.25rem", minHeight: "2.25rem" }}
        title={name || ""}
      >
        {initials}
      ...
      {isFacebook && (
        <span
          className="absolute -bottom-1 -right-1 bg-blue-600 rounded-full w-4 h-4 flex items-center justify-center"
          title="Facebook Lead"
        >
...
const SourceBadge = ({ source }) => {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${colorMap[source] || colorMap.Other}`}>
      {source}
    </span>
  );
};
...
function LeadTableComponent() {
  ...
  /* ── helpers that ONLY show a column if at least one lead has that field ── */
  const hasState   = leads.some((l) => l.state   && l.state.trim()   !== "");
  const hasCountry = leads.some((l) => l.country && l.country.trim() !== "");
        ...
        <table className="min-w-max w-full table-auto divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr className="whitespace-nowrap">
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Contact</th>
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Destination</th>
              {/* State column — only shown if any lead has a state */}
              {hasState && (
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">State</th>
              )}
              {/* Country column — only shown if any lead has a country */}
              {hasCountry && (
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Country</th>
              )}
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Source</th>
            ...
            {leads.length > 0 ? leads.map((lead, idx) => {
              ...
              return (
                <tr key={lead._id} className={`hover:bg-blue-50/30 transition-colors whitespace-nowrap ${selectedLeads.includes(lead._id) ? "bg-blue-50" : idx % 2 === 0 ? "bg-white" : "bg-gray-50/50"}`}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <LeadAvatar name={lead.leadName} isFacebook={isFacebook} />
                      <div className="flex flex-col min-w-0">
                        <span onClick={() => navigate(`/leads/view/${lead._id}`)}
                          className="font-medium text-blue-600 text-sm cursor-pointer hover:underline truncate max-w-[160px]" title={lead.leadName || "Unnamed Lead"}>
                          {lead.leadName || "Unnamed Lead"}
          ...
          {selectedLead && (
            <div className="px-6 py-5 space-y-5 max-h-[80vh] overflow-y-auto">
              ...
              <div className="p-3 bg-blue-50 rounded-lg border border-blue-100">
                <p className="text-sm text-blue-800">
                  ...
                  {selectedLead.source === "Facebook" && (
                    <span className="ml-2 inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-200 text-blue-800">
                      <Facebook className="w-3 h-3" /> Facebook Lead
                    </span>
                  )}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1.5">Land Part</label>
                  <input type="text" placeholder="e.g. 5000 or Included" value={dealData.purchasingLandCost}
                    onChange={(e) => setDealData((p) => ({ ...p, purchasingLandCost: e.target.value }))}
                    className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-400 focus:outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1.5">Ticket</label>
                  <input type="text" placeholder="e.g. 2000 or N/A" value={dealData.purchasingTicketCost}
                    onChange={(e) => setDealData((p) => ({ ...p, purchasingTicketCost: e.target.value }))}
                    className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-400 focus:outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1.5">Land Part</label>
                  <input type="text" placeholder="e.g. 7000 or Included" value={dealData.sellingLandCost}
                    onChange={(e) => setDealData((p) => ({ ...p, sellingLandCost: e.target.value }))}
                    className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-400 focus:outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1.5">Ticket</label>
                  <input type="text" placeholder="e.g. 3000 or N/A" value={dealData.sellingTicketCost}
                    onChange={(e) => setDealData((p) => ({ ...p, sellingTicketCost: e.target.value }))}
                    className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-400 focus:outline-none" />
                </div>
              </div>
